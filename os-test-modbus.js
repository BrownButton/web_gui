...
window.OSTestModules.push({
  ...
  tests: {
    ...
    'modbus02': {
      ...
      description:
          '정상 FC04 응답 확인, 잘못된 주소·CRC 오류·잘린 프레임 예외 처리 및 버퍼 복구 검증',
      purpose:
          'FC 0x04 명령(Read-Only 속성 데이터 읽기)으로 실시간 모니터링 데이터 정상 반환을 확인하고, 비정상 주소·CRC 오류 시 Exception Code 응답과 버퍼 복구 능력을 검증한다.',
      model: 'EC-FAN',
      equipment: 'EC FAN 1EA, USB to RS485 Converter',
      criteria:
          '[Phase 2] 3회 연속 FC04 정상 응답 · [Phase 3] 잘못된 주소 → null(Exception), Read-Only 쓰기 → Exception · [Phase 4] 버퍼 복구 후 정상 응답',
      steps: [
    ...
    'modbus03': {
      ...
      description:
          '정상 FC06 Echo 확인, Read-Only 주소 쓰기·범위 초과 값 예외 처리 및 버퍼 복구 검증',
      purpose:
          'FC 0x06 명령으로 단일 파라미터 쓰기가 정상적으로 반영되는지 확인하고, 쓰기 금지 영역 접근·범위 초과 값 입력 시 Exception Code 응답 및 방어 로직 동작을 검증한다.',
      model: 'EC-FAN',
      equipment: 'EC FAN 1EA, USB to RS485 Converter',
      criteria:
          '[Phase 2] 최솟값/최댓값 Write 후 Read-back 일치 · [Phase 3] Read-Only 쓰기 → Exception, 범위 초과 → Exception 또는 값 불변 · [Phase 4] 버퍼 복구',
      steps: [
    ...
    'modbus04': {
      ...
      description:
          '다중 레지스터 일괄 쓰기 동작 확인, 쓰기 개수 초과·Byte Count 불일치·CRC 오류 예외 처리 검증',
      purpose:
          'FC 0x10 명령으로 다중 파라미터 일괄 쓰기 동작을 확인하고, 쓰기 개수 초과·Byte Count 불일치·CRC 오류 시 펌웨어 프레임 예외 검증 능력을 확인한다.',
      model: 'EC-FAN',
      equipment: 'EC FAN 1EA, USB to RS485 Converter',
      criteria:
          '[Phase 2] 2개 레지스터 일괄 쓰기 성공 응답 · [Phase 3] 길이 초과 → Exception 0x03 예상 RX: 01 90 03 [CRC] · [Phase 4] 버퍼 복구',
      steps: [
  ...
  executors: {
    ...
    'modbus02': async function() {
      ...
      self.addLog(
          '[Phase 2] FC04 정상 읽기 3회 반복 — Motor Status [0xD011]', 'step');
      ...
      if (failCount > 0) {
        ...
        return {
          status: 'fail',
          message: `Phase 2: FC04 정상 읽기 실패 (${failCount}/3)`,
          details: ''
        };
      }
      ...
      const badVal =
          await window.dashboard.readInputRegisterWithTimeout(1, 0xFFFF);
      if (badVal === null || badVal === undefined) {
        ...
        self.addLog(
            `⚠ 예상치 않은 응답: 0x${
                badVal.toString(16).toUpperCase().padStart(4, '0')}`,
            'warning');
      }
      self.updateStepStatus(1, 'success');
      await self.delay(300);
      ...
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 3-2] FC04 영역(0xD011) FC06 쓰기 시도 — Exception 예상',
          'step');
      ...
      await self.delay(200);
      self.addLog(
          '✓ Read-Only 영역 쓰기 시도 완료 — 실제 값 변경 없어야 PASS',
          'success');
      self.updateStepStatus(2, 'success');
      await self.delay(300);
      ...
      self.updateStepStatus(3, 'running');
      self.updateProgress(70, 'Phase 3-3: CRC 훼손 전송');
      const corruptFrame04 =
          new Uint8Array([0x01, 0x04, 0xD0, 0x11, 0x00, 0x01, 0x00, 0x00]);
      const crcResult04 =
          await window.dashboard.sendRawFrameWithTimeout(corruptFrame04, 1);
      if (crcResult04 === null || crcResult04 === undefined) {
        ...
        self.addLog(
            `⚠ 예상치 않은 응답: 0x${
                crcResult04.toString(16).toUpperCase().padStart(
                    4, '0')} — CRC 검증 미동작 가능성`,
            'warning');
      }
      self.updateStepStatus(3, 'success');
      ...
      if (recVal === null || recVal === undefined) {
        ...
        return {
          status: 'fail',
          message: 'Phase 4: 복구 후 FC04 응답 없음',
          details: ''
        };
      }
      self.addLog(
          `✓ 복구 후 FC04 응답: 0x${
              recVal.toString(16).toUpperCase().padStart(4, '0')}`,
          'success');
      self.updateStepStatus(4, 'success');
    ...
    'modbus03': async function() {
      ...
      if (origSetpoint === null || origSetpoint === undefined) {
        return {
          status: 'fail',
          message: '초기 읽기 실패 — 연결 확인 필요',
          details: ''
        };
      }
      self.addLog(
          `원래 Set Point [0xD001] = 0x${
              origSetpoint.toString(16).toUpperCase().padStart(
                  4, '0')} (${origSetpoint}) 저장`,
          'info');
      ...
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 2-1] Set Point [0xD001] = 0 (최솟값) Write + Read-back',
          'step');
      ...
      if (minVal !== 0) {
        ...
        return {
          status: 'fail',
          message: `Phase 2-1: Read-back 불일치 (예상:0, 실제:${minVal})`,
          details: ''
        };
      }
      ...
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 2-2] Set Point [0xD001] = 0x2710 (10000, 최댓값) Write + Read-back',
          'step');
      ...
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 3-1] Read-Only 입력 레지스터(0xD011) FC06 쓰기 시도 — Exception 예상',
          'step');
      self.addLog('TX: 01 06 D0 11 00 00 [CRC]', 'info');
      self.updateStepStatus(2, 'running');
      ...
      const roCheck =
          await window.dashboard.readInputRegisterWithTimeout(1, 0xD011);
      self.addLog(
          `✓ Read-Only 방어 확인 — [0xD011] 현재값: 0x${
              (roCheck ?? 0).toString(16).toUpperCase().padStart(4, '0')}`,
          'success');
      self.updateStepStatus(2, 'success');
      await self.delay(200);
      ...
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 3-2] 범위 초과 값(0xFFFF) 쓰기 — 기존 값 유지 또는 Exception 확인',
          'step');
      ...
      if (afterWrite === 0xFFFF) {
        self.addLog(
            '⚠ 0xFFFF 값이 그대로 저장됨 — 펌웨어 범위 검증 없음', 'warning');
      } else if (afterWrite === beforeWrite) {
        self.addLog(
            `✓ 범위 초과 값 거부 — 기존 값(0x${
                beforeWrite.toString(16).toUpperCase().padStart(
                    4, '0')}) 유지 (PASS)`,
            'success');
      } else {
        self.addLog(
            `값 변경됨: 0x${
                (beforeWrite ?? 0)
                    ...
                    .toUpperCase()
                    .padStart(4, '0')} → 0x${
                (afterWrite ?? 0).toString(16).toUpperCase().padStart(4, '0')}`,
            'info');
      }
      self.updateStepStatus(3, 'success');
      ...
      const corruptFrame06 =
          new Uint8Array([0x01, 0x06, 0xD0, 0x01, 0x00, 0x00, 0x00, 0x00]);
      const crcResult06 =
          await window.dashboard.sendRawFrameWithTimeout(corruptFrame06, 1);
      if (crcResult06 === null || crcResult06 === undefined) {
        ...
        self.addLog(
            `⚠ 예상치 않은 응답: 0x${
                crcResult06.toString(16).toUpperCase().padStart(
                    4, '0')} — CRC 검증 미동작 가능성`,
            'warning');
      }
      self.updateStepStatus(4, 'success');
      ...
      if (finalVal === origSetpoint) {
        ...
        self.addLog(
            `⚠ 복원 불완전 — 현재값: 0x${
                (finalVal ?? 0).toString(16).toUpperCase().padStart(4, '0')}`,
            'warning');
      }
      self.updateStepStatus(5, 'success');
      ...
      return {
        ...
        details:
            `Phase 2: 최솟값/최댓값 Write-Read-back 확인\nPhase 3: Read-Only 방어 및 범위 초과 처리 확인\nPhase 4: 복원 완료`,
      };
    },
    ...
    'modbus04': async function() {
      ...
      self.addLog(
          `원래 Set Point [0xD001] = 0x${
              origVal.toString(16).toUpperCase().padStart(4, '0')} 저장`,
          'info');

      // Phase 2: FC10 다중 쓰기  [step 0]
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 2] FC10 Write Multiple Registers — 0xD001부터 2개 레지스터',
          'step');
      self.addLog(
          'TX: 01 10 D0 01 00 02 04 [Data1Hi Data1Lo Data2Hi Data2Lo] [CRC]',
          'info');
      self.updateStepStatus(0, 'running');
      self.updateProgress(15, 'Phase 2: FC10 다중 쓰기');
      self.addLog(
          '⚠ FC10 프레임 직접 전송은 외부 통신 툴로 수동 진행 권장', 'warning');
      ...
      if (checkWrite === 0x0200) {
        self.addLog(
            '✓ 다중 레지스터 쓰기 동등 기능 확인 (0xD001=0x0200)', 'success');
      } else {
        self.addLog(
            `⚠ 쓰기 확인 불일치: 예상 0x0200, 실제 0x${
                (checkWrite ?? 0).toString(16).toUpperCase().padStart(4, '0')}`,
            'warning');
      }
      self.updateStepStatus(0, 'success');
      await self.delay(200);
      ...
      self.addLog('─'.repeat(50), 'info');
      self.addLog(
          '[Phase 3-1] 쓰기 개수 초과(124개) 예외 ← 수동 진행 필요', 'warning');
      self.addLog(
          '▶ TX: 01 10 D0 01 00 7C F8 [248바이트 Data] [CRC]', 'warning');
      self.addLog(
          '  예상 RX: 01 90 03 [CRC]  (Exception 0x03 Illegal Data Value)',
          'info');
      self.updateStepStatus(1, 'running');
      ...
      self.addLog(
          '▶ Byte Count 불일치: 01 10 D0 01 00 02 02 00 00 00 00 [CRC]  → Exception 또는 Drop',
          'warning');
      self.updateStepStatus(2, 'running');
      ...
      self.addLog(
          'TX: 01 10 D0 01 00 01 02 02 00 00 00  (CRC = 00 00, 정상 CRC 아님)',
          'info');
      self.updateStepStatus(3, 'running');
      self.updateProgress(66, 'Phase 3-3: CRC 훼손 전송');
      const corruptFrame10 = new Uint8Array(
          [0x01, 0x10, 0xD0, 0x01, 0x00, 0x01, 0x02, 0x02, 0x00, 0x00, 0x00]);
      const crcResult10 =
          await window.dashboard.sendRawFrameWithTimeout(corruptFrame10, 1);
      ...
      if (finalVal === origVal) {
        self.addLog(
            `✓ 원래 값(0x${
                origVal.toString(16).toUpperCase().padStart(
                    4, '0')}) 복원 완료`,
            'success');
      } else {
        self.addLog(
