...
class ChartManager {
  ...
  handleMouseMove(e) {
    ...
    if (this.isDragging) {
      ...
      this.viewMinTime = Math.max(
          0, Math.min(maxMinTime, this.panStartViewMinTime + deltaTime));
  ...
  handleClick(e) {
    ...
    this.markers.push({x: chartX, screenX: x});
  ...
  screenToChartX(screenX) {
    const chartWidth =
        this.width - this.chartMargins.left - this.chartMargins.right;
    const relX = (screenX - this.chartMargins.left - this.pan.x) /
  ...
  screenToChartY(screenY) {
    ...
    const relY = 1 -
        (screenY - this.chartMargins.top - this.pan.y) /
            (chartHeight * this.zoom.y);
  ...
  chartToScreenX(chartX) {
    ...
    return this.chartMargins.left + relX * chartWidth * this.zoom.x +
        this.pan.x;
  }
  ...
  chartToScreenY(chartY) {
    ...
    return this.chartMargins.top + (1 - relY) * chartHeight * this.zoom.y +
        this.pan.y;
  }
  ...
  updateCursorInfo() {
    ...
    this.channels.forEach((ch, i) => {
      ...
      itemEl.innerHTML = `
                <span class="cursor-value-dot" style="background: ${
          ch.color}"></span>
  ...
  getValueAtTime(channelIndex, timeMs) {
    ...
    return Math.abs(before.t - timeMs) <= Math.abs(after.t - timeMs) ?
        before.v :
        after.v;
  }
  ...
  updateMarkersInfo() {
    ...
    if (this.markers.length === 0) {
      ...
      infoEl.textContent =
          `M1: ${this.formatTime(this.markers[0].x)} (Click to add M2)`;
    } else {
      const delta = Math.abs(this.markers[1].x - this.markers[0].x);
      infoEl.textContent = `M1: ${this.formatTime(this.markers[0].x)} | M2: ${
          this.formatTime(this.markers[1].x)} | Δ: ${this.formatTime(delta)}`;
    }
  }
  ...
  handleTriggerMode(channelIndex, point) {
    ...
    if (channelIndex === this.trigger.channel) {
      ...
      const prevValue =
          prevData.length > 0 ? prevData[prevData.length - 1].v : null;

      let triggered = false;
      if (prevValue !== null) {
        if (this.trigger.edge === 'rising' && prevValue < this.trigger.level &&
            point.v >= this.trigger.level) {
          triggered = true;
        } else if (
            this.trigger.edge === 'falling' && prevValue > this.trigger.level &&
            point.v <= this.trigger.level) {
          triggered = true;
        } else if (
            this.trigger.edge === 'both' &&
            ((prevValue < this.trigger.level &&
              point.v >= this.trigger.level) ||
             (prevValue > this.trigger.level &&
              point.v <= this.trigger.level))) {
          triggered = true;
    ...
    if (this.trigger.triggered) {
      ...
      if (mainChannel.data.length >=
          this.trigger.preSamples + this.trigger.postSamples) {
        this.stopCapture();
      }
    }
  }
  ...
  updateStats() {
    ...
    if (ptsEl)
      ptsEl.textContent = totalSamples >= 1000 ?
    ...
    const animate = () => {
      ...
      if (!this.isPaused) {
        if (this.autoScroll && this.mode === 'continuous') {
          ...
          // If the jump is within this threshold it's normal streaming → snap
          // directly (no lag). If it's a large burst arrival → rate-limit to
          // smooth the jump over a few frames.
          const threshold = this.timeScale / 60;
          if (diff <= threshold) {
            // Normal case: snap (covers data arriving faster OR slower than
            // wall clock)
  ...
  drawAxes() {
    ...
    if (this.yAxisMode === 'independent') {
      ...
      activeChannels.forEach(({ch}, activeIdx) => {
        ...
        const decimals = Math.abs(range) >= 100 ? 0 :
            Math.abs(range) >= 10               ? 1 :
                                                  2;
  ...
  drawData() {
    ...
    for (const ch of this.channels) {
      ...
      // Binary search: one point before and after visible range for smooth line
      // entry/exit
      const startIdx = Math.max(0, this._bisectLeft(ch.data, visMin) - 1);
      const endIdx =
          Math.min(ch.data.length - 1, this._bisectRight(ch.data, visMax) + 1);
      ...
      for (let k = startIdx; k <= endIdx; k++) {
        ...
        if (this.yAxisMode === 'independent') {
          ...
          const normalized =
              range !== 0 ? ((point.v - (ch.chYMin ?? 0)) / range) * 100 : 50;
  ...
  drawTriggerLevel() {
    ...
    const pickerVal = parseInt(
        document.getElementById('triggerSourcePicker')?.dataset.selectedValue ??
        255);
    if (!isNaN(levelVal) &&
        pickerVal !== 255) {  // Immediate(0xFF=255)가 아닌 경우에만 레벨선 표시
      let y;
      if (this.yAxisMode === 'independent') {
...
class PolarChart {
  ...
  addSample(rawU, rawV, rawW) {
    ...
    if (this._a.length > this.maxPoints) {
      this._a.shift();
      this._b.shift();
    }
  }
  ...
  render() {
    ...
    const LABEL_ANGLES =
        [30, 55];  // degrees, 첫번째=평상시, 두번째=Offset 조정
    ...
    if (n < 2) {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(
          '데이터 대기 중… (카드 클릭하여 시작)', cx, cy + radius * 1.18);
      return;
    }
...
class ModbusDashboard {
  constructor() {
    ...
    this._serialDataCb =
        null;  // event-driven RX callback for sendAndReceiveFC64
    ...
    // Settings
    this.baseFrameCount =
        1000;  // Default base frame count for error rate calculation
    ...
    this.deviceViewMode =
        localStorage.getItem('deviceViewMode') || 'card';  // 'card' or 'list'
    ...
    this.MOTOR_STATUS_BITS = {
      UzLow: {
        mask: 0x1000,
        name: 'DC Undervolt',
        description: 'DC-link undervoltage'
      },
      RL_Cal: {
        mask: 0x0400,
        name: 'Rotor Cal',
        description: 'Rotor position sensor calibration error'
      },
      n_Limit: {
        mask: 0x0100,
        name: 'Speed Limit',
      ...
      FB: {
        mask: 0x0010,
        name: 'Fan Bad',
        description: 'Fan Bad (general error)'
      },
      SKF: {
        mask: 0x0008,
        name: 'Comm Error',
        description: 'Communication error between master and slave controller'
      },
      TFE: {
        mask: 0x0004,
        name: 'Stage OT',
        description: 'Output stage overheated'
      },
      PHA: {
        mask: 0x0001,
        name: 'Phase Fail',
        description: 'Phase failure or line undervoltage'
      }
    };
    ...
    this.commandQueue = [];  // Queue for user write commands - prevents 485 bus
                             // collision during polling
    this._busBusy =
        false;  // Bus Mutex: true이면 TX/RX 쌍 진행 중 — 동시 TX 원천 차단
    this._tabSeq =
        0;  // Tab Context Token: 서브탭 전환 시 증가 — stale response 감지용
    this._paramReadProgress =
        null;  // 진행 중인 readAllParameters 프로그레스 컨트롤러
    this.pollingTimeout = 200;  // Response timeout in ms for polling
    this.commandTimeout =
        200;  // Response timeout in ms for user commands (write/read)
    ...
    this._currentRmsData =
        [{sumSq: 0, count: 0}, {sumSq: 0, count: 0}, {sumSq: 0, count: 0}];
  ...
  initializeUI() {
    ...
    sidebarToggle.addEventListener('click', () => {
      ...
      if (mainContent) {
        mainContent.style.marginLeft =
            sidebar.classList.contains('collapsed') ? '0' : '280px';
      }
    });
    ...
    document.getElementById('sidebar-connectBtn')
    ...
    const modbusInputs = [
      ...
      {id: 'quantity', min: 1, max: 125}, {id: 'writeValue', min: 0, max: 65535}
    ];
    ...
    document.getElementById('fc2bOperation').addEventListener('change', (e) => {
      ...
      document.getElementById('fc2bNumDataGroup').style.display =
          isWrite ? 'none' : 'flex';
      document.getElementById('fc2bWriteGroup').style.display =
          isWrite ? 'flex' : 'none';
    });

    // Parameter management
    document.getElementById('addParamBtn')
        .addEventListener('click', () => this.showAddParameterModal());
    document.getElementById('cancelParamBtn')
        .addEventListener('click', () => this.hideAddParameterModal());
    document.getElementById('saveParamBtn')
        .addEventListener('click', () => this.saveParameter());
    ...
    document.getElementById('loadDefaultCsvBtn')
        .addEventListener('click', () => this.loadDefaultCSV());
    ...
    document.querySelectorAll('.filter-btn[data-implemented]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        document.querySelectorAll('.filter-btn[data-implemented]')
            .forEach(b => b.classList.remove('active'));
        e.target.classList.add('active');
    ...
    addParamModal.addEventListener('click', (e) => {
      if (e.target.id === 'addParamModal' && paramModalMouseDownTarget &&
          paramModalMouseDownTarget.id === 'addParamModal') {
    ...
    const paramDeviceRadioGroup =
        document.getElementById('paramDeviceRadioGroup');
    ...
    const paramDeviceHeaderLeft =
        document.querySelector('.param-device-header-left');
    const paramDeviceSelector =
        document.querySelector('.param-device-selector');
    if (paramDeviceHeaderLeft && paramDeviceSelector) {
      ...
      // Auto-collapse when mouse leaves (only if not fixed and a device is
      // selected)
    ...
    // Simulator controls
    document.getElementById('simToggleBtn')
        .addEventListener('click', () => this.toggleSimulator());
    document.getElementById('simResetBtn')
        .addEventListener('click', () => this.resetSimulator());
    ...
    // Settings controls
    document.getElementById('baseFrameCount')
   ...
   */
  handleStatsOutsideClick =
      (e) => {
        const navbarStats = document.getElementById('navbarStats');
        const statsTooltip = document.getElementById('navbarStatsTooltip');

        if (navbarStats && statsTooltip && !navbarStats.contains(e.target) &&
  ...
  syncSerialSettings() {
    ...
    if (sidebarBaudRate)
      sidebarBaudRate.value = String(DEFAULT_SERIAL.baudRate);
    if (sidebarParity) sidebarParity.value = DEFAULT_SERIAL.parity;
    if (sidebarDataBits)
      sidebarDataBits.value = String(DEFAULT_SERIAL.dataBits);
    if (sidebarStopBits)
      sidebarStopBits.value = String(DEFAULT_SERIAL.stopBits);
  ...
  initDeveloperMode() {
    ...
    const pageToShow =
        (savedPage === 'firmware' ? 'device-setup' : savedPage) || 'dashboard';
  ...
  enableDeveloperMode() {
    ...
    const deviceManufactureTab =
        document.querySelector('.device-setup-tab[data-tab="manufacture"]');
    if (deviceManufactureTab) {
      deviceManufactureTab.style.display =
          '';  // Remove inline display:none to show the button
    }
  ...
  initMonitorPanel() {
    ...
    this.monitorPanelWidth =
        parseInt(localStorage.getItem('monitorPanelWidth')) || 500;

    // Apply saved width using CSS variable
    panel.style.width = this.monitorPanelWidth + 'px';
    document.documentElement.style.setProperty(
        '--monitor-panel-width', this.monitorPanelWidth + 'px');
    ...
    monitorDisplay.addEventListener('click', (e) => {
      ...
      if (rawContainer) {
        ...
        navigator.clipboard.writeText(rawData)
            .then(() => {
              this.showToast(
                  'Raw 데이터가 클립보드에 복사되었습니다', 'success');
            })
            .catch(() => {
  ...
  openMonitorPanel() {
    ...
    document.documentElement.style.setProperty(
        '--monitor-width', this.monitorPanelWidth + 'px');
  ...
  initSettingsModal() {
    ...
    settingsModal.addEventListener('click', (e) => {
      if (e.target === settingsModal &&
          settingsMouseDownTarget === settingsModal) {
        this.closeSettingsModal();
      }
    ...
    document.getElementById('modal-baseFrameCount')
        .addEventListener('change', (e) => {
          this.baseFrameCount = parseInt(e.target.value);
          this.saveSettings();
          this.updateStatsDisplay();
    ...
    document.getElementById('modal-simToggleBtn')
        .addEventListener('click', () => this.toggleSimulator());
    document.getElementById('modal-simResetBtn')
        .addEventListener('click', () => this.resetSimulator());
    document.getElementById('modal-simSlaveId')
        .addEventListener('change', (e) => {
          this.simulator.slaveId = parseInt(e.target.value);
          document.getElementById('simSlaveId').value = e.target.value;
        });
    document.getElementById('modal-simDelay')
        .addEventListener('change', (e) => {
          this.simulator.responseDelay = parseInt(e.target.value);
    ...
    // Polling settings
    document.getElementById('applyPollingSettings')
        .addEventListener('click', () => {
   ...
   * 공식: 디바이스당 최악 주기 = (읽기 타임아웃 × 2 + 메시지 간격 × 3) + 폴링
   * 간격 "최악"은 두 번의 읽기(status, speed)가 모두 타임아웃 되는 경우
   */
  updatePollingEstimate() {
    ...
    const readTimeout =
        parseInt(document.getElementById('pollingTimeout')?.value) ||
        this.pollingTimeout;
    const gap = parseInt(document.getElementById('paramPollingDelay')?.value) ||
        this.paramPollingDelay;
    const pollInterval =
        parseInt(document.getElementById('pollingInterval')?.value) ||
        this.autoPollingInterval;
    const numDevices = this.devices ? this.devices.length : 0;

    // 디바이스 1대당 1 사이클: status read + gap + speed read + gap + (모니터링
    // 파라미터) + pollInterval 모니터링 파라미터 수는 평균으로 추정
    const avgMonParams = numDevices > 0 ?
        this.devices.reduce(
            (s, d) => s + (d.monitoringParams?.length || 0), 0) /
            numDevices :
        0;
    const readsPerDevice =
        2 + Math.round(avgMonParams);  // status + speed + monitoring
    const worstPerDevice =
        readTimeout * readsPerDevice + gap * readsPerDevice + pollInterval;
    const normalPerDevice =
        gap * readsPerDevice + pollInterval;  // 응답이 빠를 때 (응답시간 무시)
      ...
      el.innerHTML = `
          ...
          fmt(normalRound)}</div>
                    ...
                    <div>
                        ...
                        <div style="color:#f08080; font-weight:600;">최대 ${
          fmt(worstRound)}</div>
                    </div>
                </div>`;
    }
  ...
  applyPollingSettings() {
    ...
    const readTimeout =
        parseInt(document.getElementById('pollingTimeout').value);
    const writeTimeout =
        parseInt(document.getElementById('commandTimeout').value);
    const paramDelay =
        parseInt(document.getElementById('paramPollingDelay').value);
  ...
  loadPollingSettings() {
    ...
    if (saved) {
      try {
        ...
        if (settings.autoPollingInterval)
          this.autoPollingInterval = settings.autoPollingInterval;
        if (settings.pollingTimeout)
          this.pollingTimeout = settings.pollingTimeout;
        if (settings.commandTimeout)
          this.commandTimeout = settings.commandTimeout;
        if (settings.paramPollingDelay !== undefined)
          this.paramPollingDelay = settings.paramPollingDelay;
        if (settings.backgroundPollingEnabled !== undefined)
          this.backgroundPollingEnabled = settings.backgroundPollingEnabled;
      } catch (e) {
  ...
  openSettingsModal() {
    ...
    document.getElementById('modal-simDelay').value =
        this.simulator.responseDelay;
    ...
    document.getElementById('backgroundPollingToggle').checked =
        this.backgroundPollingEnabled;
    this.updatePollingEstimate();
  ...
  async connect() {
    try {
      ...
      const settingsStr = `${baudRate} baud, ${dataBits}${
          parity.charAt(0).toUpperCase()}${stopBits}`;
      this.addMonitorEntry('received', `Connected: ${settingsStr}`);
      this.showToast(
          `시리얼 포트가 연결되었습니다 (${settingsStr})`, 'success');
      ...
      // 처리 auto-scan이 비활성화된 경우: 폴링 시작 후 바로 실행
      if (!this.autoScanEnabled) {
  ...
  loadSerialSettings() {
    ...
    if (saved) {
      try {
        ...
        if (baudRateEl && settings.baudRate)
          baudRateEl.value = settings.baudRate;
        if (dataBitsEl && settings.dataBits)
          dataBitsEl.value = settings.dataBits;
        if (parityEl && settings.parity) parityEl.value = settings.parity;
        if (stopBitsEl && settings.stopBits)
          stopBitsEl.value = settings.stopBits;
  ...
  async disconnect() {
    try {
      ...
      if (this.reader) {
        ...
        } catch (e) { /* ignore - port may be gone */
        }
        // startReading()의 finally 블록에서 이미 releaseLock()이 호출될 수
        // 있으므로 재확인
        if (this.reader) {
          try {
  ...
  async reconnectSerial(baudRate, parity, stopBits = 1, silent = false) {
    ...
    await port.open(
        {baudRate, parity, stopBits, dataBits: 8, flowControl: 'none'});
    ...
    if (!silent)
      this.showToast(
          `재접속 완료: ${baudRate}bps, ${parity}, Stop${stopBits}`, 'success');
  }
  ...
  async startReading() {
    ...
    try {
      ...
      if (this.readInProgress) {
        ...
        // 예기치 않은 읽기 오류 → 충돌 원인인지 물리 제거인지 판단 후 자동
        // 재연결 시도
        setTimeout(() => {
          if (this.isConnected) {
      ...
      if (this.reader) {
        ...
        try {
          await this.reader.cancel();
        } catch (e) { /* stream already errored — ignore */
        }
        this.reader = null;
      }
    }
  ...
  async autoReconnectAfterCollision() {
    ...
    if (!saved) {
      this._autoReconnecting = false;
      this.addMonitorEntry(
          'error', 'Serial port physically removed - auto disconnecting');
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        this.addMonitorEntry(
            'error',
            `버스 충돌 감지 — 재연결 시도 ${attempt}/${MAX_RETRIES} (${
                delay}ms 후)`);
        ...
        await port.open(
            {baudRate, dataBits, parity, stopBits, flowControl: 'none'});
        ...
        // 대시보드 페이지라면 폴링 재개
        if (this.currentPage === 'dashboard' &&
            this.devices.some(d => d.slaveId !== 0)) {
        ...
        this.addMonitorEntry(
            'error', `재연결 시도 ${attempt} 실패: ${e.message}`);
      }
    }
  ...
  getExpectedFrameLength() {
    ...
    // FC 05, 06, 0F, 10 쓰기 응답 에코: slave(1) + fc(1) + addr(2) +
    // value/qty(2) + CRC(2) = 8
  ...
  tryParseFrame() {
    ...
    // Modbus 유효 슬레이브 주소는 1~247 (0x01~0xF7)이므로 범위 밖 바이트는
    // 무효. pendingResponse가 있으면 기대 slaveId와 다른 선두 바이트도 노이즈로
    // 처리.
    {
      const expectedId =
          this.pendingResponse ? this.pendingResponse.slaveId : null;
    ...
    if (this.receiveBuffer[1] === 0x2B &&
        ...
        this.modbus.verifyCRC(
            this.receiveBuffer.slice(0, expectedLength - 1))) {
      actualLength = expectedLength - 1;
      fc2bLengthShort =
          true;  // 펌웨어 버그: NumData 선언보다 1바이트 짧게 수신
    ...
    // FC 0x2B 길이 부족 경고 (CRC는 유효하나 NumData 선언보다 1바이트 짧게
    // 수신) 별도 error 엔트리 대신 다음 received 엔트리에 경고 sub-line을 붙임
    if (fc2bLengthShort) {
      const declared = (frame[11] << 8) | frame[12];
      this._pendingRxWarning =
          `⚠ FC 0x2B: RX length mismatch — NumData declares ${
              declared} bytes but received ${
              declared - 1} (short by 1). Firmware bug suspected.`;
    }
    ...
    if (this.scanResolve) {
      ...
      if (responseSlaveId === this.scanExpectedSlaveId &&
          (functionCode & 0x80) === 0) {
  ...
  /**
   * Parse Modbus input value (supports decimal and hexadecimal with
   * auto-detection)
  ...
  parseModbusValue(value, min = 0, max = 65535) {
    ...
    if (parsed < min || parsed > max) {
      throw new Error(`값 ${parsed} (0x${
          parsed.toString(16).toUpperCase()})는 범위를 벗어났습니다 (${min}-${
          max})`);
    }
  ...
  /**
   * Update converted value display for an input field (ID-based, for Modbus
   * tab)
  ...
  addMonitorEntry(type, dataOrMessage, parsedData = null, errorMsg = null) {
    // FC 0x2B 펌웨어 버그 워크어라운드: 경고 메시지를 다음 received 엔트리에
    // one-shot으로 붙임
    ...
    const entryData = {
      type,
      dataOrMessage: dataOrMessage instanceof Uint8Array ?
          new Uint8Array(dataOrMessage) :
          dataOrMessage,
      parsedData,
      errorMsg,
      timeStr,
    ...
    // Limit array size to prevent memory bloat (keep last 500 entries in
    // memory)
    ...
    if (excessCount > 0) {
      ...
      if (this.isUserScrolling) {
        ...
        if (currentScrollTop > 0) {
          monitorDisplay.scrollTop =
              Math.max(0, currentScrollTop - removedHeight);
        }
  ...
  createMonitorEntryElement(entryData) {
    const {type, dataOrMessage, parsedData, errorMsg, timeStr, deltaStr} =
        entryData;
    ...
    if (type === 'sent' || type === 'tx')
      typeLabel = 'TX';
    else if (type === 'received' || type === 'rx')
      typeLabel = 'RX';
    else if (type === 'error')
      typeLabel = 'ERROR';
    ...
    if (dataOrMessage instanceof Uint8Array) {
      ...
      Array.from(dataOrMessage).forEach((byte, index) => {
        const info = byteInfo[index] ||
            {name: 'Unknown', desc: '', class: 'byte-unknown'};
      ...
      summary.textContent =
          this.getFrameSummary(dataOrMessage, parsedData, type);
      ...
      if (errorMsg) {
        ...
        warnLine.style.cssText =
            'font-size: 11px; color: #e67e00; padding: 2px 0 0 4px;';
      ...
      entry._detailsData =
          {frame: dataOrMessage, byteInfo, parsedData, type, timeStr, deltaStr};
    } else {
    ...
    if (entry.dataset.hasDetails === 'true' && entry._detailsData) {
      ...
      details.innerHTML =
          this.createDetailsHTML(frame, byteInfo, pData, pType, pTime, pDelta);
  ...
  initMonitorScrollControls() {
    ...
    monitorDisplay.addEventListener('scroll', () => {
      ...
      const currentScrollTop = monitorDisplay.scrollTop;
      const isAtBottom = monitorDisplay.scrollHeight - currentScrollTop -
              monitorDisplay.clientHeight <
          50;
      ...
      scrollTimeout = setTimeout(() => {
        const stillAtBottom = monitorDisplay.scrollHeight -
                monitorDisplay.scrollTop - monitorDisplay.clientHeight <
            50;
  ...
  createDetailsHTML(frame, _byteInfo, _parsedData, type, timeStr, deltaStr) {
    ...
    if (!frame || frame.length < 2) {
      return `<div class="monitor-details-grid">
                ...
                <div class="monitor-detail-item detail-raw">
                    ...
                    <div class="monitor-detail-value">${
    ...
    detailsHTML += `
            ...
            <div class="monitor-detail-item detail-func">
                ...
                <div class="monitor-detail-value">${isError ? 'Error' : ''} ${
        funcNames[actualFunc] || 'Unknown'} (0x${
        actualFunc.toString(16).toUpperCase().padStart(2, '0')})</div>
            </div>
        `;
    ...
    if (frame.length >= 6 && !isError) {
      ...
      detailsHTML += `
                <div class="monitor-detail-item detail-addr">
                    ...
                    <div class="monitor-detail-value">0x${
          addr.toString(16).toUpperCase().padStart(4, '0')} (${addr})</div>
                </div>
            `;
    ...
    const hexString =
        Array.from(frame)
            ...
            .join(' ');
    detailsHTML += `
            <div class="monitor-raw-container" data-raw="${
        hexString}" title="클릭하여 복사">
                <strong>Raw:</strong> <span class="monitor-raw-value">${
        hexString}</span>
  ...
  getByteInfo(frame, parsedData, type) {
    ...
    info[1] = {
      ...
      desc: isError ?
          `에러 응답 (원본: ${funcNames[actualFunc] || 'Unknown'})` :
          funcNames[actualFunc] || 'Unknown',
      value: frame[1],
    ...
    if (isError && isResponse) {
      ...
      info[2] = {
        name: 'Exception Code',
        desc: exceptionNames[frame[2]] || 'Unknown Error',
        value: frame[2],
        class: 'byte-error',
        size: 1
      };
      ...
      if ([1, 2, 3, 4].includes(funcCode)) {
        ...
        info[4] = {
          name: 'Coil Value',
          desc: val === 0xFF00 ? 'ON (0xFF00)' : 'OFF (0x0000)',
          value: val,
          class: 'byte-data',
          size: 2,
          group: [4, 5]
      ...
      if ([1, 2].includes(actualFunc)) {
        ...
        for (let i = 3; i < 3 + byteCount && i < len - 2; i++) {
          const bits =
              frame[i].toString(2).padStart(8, '0').split('').reverse().join(
                  '');
          info[i] = {
            name: `Coil Data[${i - 3}]`,
            desc: `비트: ${bits}`,
        ...
        info[2] = {
          name: 'Byte Count',
          desc: `${byteCount}바이트 (${byteCount / 2}개 레지스터)`,
          value: byteCount,
          class: 'byte-count',
          size: 1
        };
  ...
  getFrameSummary(frame, parsedData, type) {
    ...
    if (type === 'sent') {
      if ([1, 2, 3, 4].includes(funcCode)) {
        ...
        summary += ` @${addr}=${
            funcCode === 5 ? (value === 0xFF00 ? 'ON' : 'OFF') : value}`;
      }
    } else {
  ...
  showByteTooltip(e, info, value, index) {
    ...
    // Byte position text
    const byteText =
        info.group ? `Byte ${info.group[0]}-${info.group[1]}` : `Byte ${index}`;
  ...
  unpinByteTooltip() {
    ...
    if (this.pinnedBytesContainer && this.pinnedGroup) {
      this.highlightByteGroup(
          this.pinnedBytesContainer, this.pinnedGroup, false);
    }
  ...
  handleOutsideClick =
      (e) => {
        ...
        if (tooltip && !tooltip.contains(e.target) &&
            !e.target.classList.contains('monitor-byte')) {
          this.unpinByteTooltip();
        }
      }
  ...
  updateStatsDisplay() {
    ...
    if (errRateEl) {
      const maxChars =
          String(this.baseFrameCount).length + 2;  // e.g. 1000 → "1000.0" = 6
  ...
  updateDeviceStatsDisplay() {
    ...
    const slaveIds =
        Object.keys(this.deviceStats).map(Number).sort((a, b) => a - b);
              ...
              return `
                <div class="device-stats-row">
                    <div class="stats-device-info">
                        ...
                        <span class="device-name" data-devname="${name}">${
                  name}</span>
                    </div>
  ...
  setupDeviceNameTooltips(container) {
    ...
    if (!tip) {
      ...
      tip.style.cssText = [
        'position:fixed', 'background:rgba(44,62,80,0.95)', 'color:#fff',
  ...
  updateConnectionStatus(connected) {
    const navbarStatusIndicator =
        document.getElementById('navbar-status-indicator');
    ...
    if (connected) {
      navbarStatusIndicator.className =
          'navbar-status-indicator status-connected';
      ...
      navbarStatusIndicator.className =
          'navbar-status-indicator status-disconnected';
  ...
  saveParameter() {
    ...
    const functionCode =
        parseInt(document.getElementById('paramFunction').value);
  ...
  loadParameters() {
    ...
    if (stored) {
      ...
      // Clear previous session values - only show values actually read from
      // device
  ...
  loadSettings() {
    ...
    if (stored) {
      ...
      this.autoScanEnabled = settings.autoScanEnabled !== undefined ?
      ...
      if (scanRegister)
        scanRegister.value =
            '0x' + this.scanRegister.toString(16).toUpperCase();

      const scanRemoveNotFoundToggle =
          document.getElementById('scanRemoveNotFound');
      const scanRemoveNotFoundStatus =
          document.getElementById('scanRemoveNotFoundStatus');
      if (scanRemoveNotFoundToggle) {
        scanRemoveNotFoundToggle.checked = this.scanRemoveNotFound;
        if (scanRemoveNotFoundStatus) {
          scanRemoveNotFoundStatus.textContent =
              this.scanRemoveNotFound ? '활성' : '비활성';
          scanRemoveNotFoundStatus.classList.toggle(
              'active', this.scanRemoveNotFound);
        }
      }
    }

    // Always sync autoScanEnabled UI to match the current value
    // (handles the case where no stored settings exist but this.autoScanEnabled
    // defaults to true)
    const autoScanToggle = document.getElementById('autoScanEnabled');
    const autoScanStatus = document.getElementById('autoScanStatus');
    if (autoScanToggle) {
  ...
  updateParamDeviceSelector() {
            ...
            <input type="radio"
                   ...
                   ${
                    this.selectedParamDeviceId === device.slaveId ? 'checked' :
                                                                    ''}>
            <label class="param-device-radio-label" for="paramDevice_${
                    device.slaveId}">
    ...
    if (this.selectedParamDeviceId) {
      const stillExists =
          validDevices.some(d => d.slaveId === this.selectedParamDeviceId);
      if (!stillExists) {
        this.selectedParamDeviceId = null;
      }
    }
  ...
  updateParamDeviceStatus() {
    ...
    if (this.selectedParamDeviceId) {
      const device =
          this.devices.find(d => d.slaveId === this.selectedParamDeviceId);
  ...
  async readAllParameters() {
    ...
    if (!this.writer && !this.simulatorEnabled) {
      this.showToast(
          '시리얼 포트에 연결하거나 시뮬레이터를 활성화하세요', 'warning');
      return;
    }
    ...
    const implementedParams = this.parameters.filter(p => {
      ...
      if (this.paramTypeFilter !== 'all' && p.type !== this.paramTypeFilter)
        return false;
      if (this.paramImplementedFilter !== 'all' &&
          p.implemented !== this.paramImplementedFilter)
        return false;
    ...
    this._paramReadProgress =
        this.showProgressToast(`Reading ${total} parameters...`, total);
  ...
  renderParameters() {
    ...
    let filtered = this.parameters.filter(param => {
      // Type filter
      if (this.paramTypeFilter !== 'all' &&
          param.type !== this.paramTypeFilter) {
        return false;
      }
      // Implemented filter
      if (this.paramImplementedFilter !== 'all' &&
          param.implemented !== this.paramImplementedFilter) {
        return false;
      }
      // Search filter
      if (this.paramSearchText) {
    ...
    if (paramCount) {
      paramCount.textContent =
          `${filtered.length} / ${this.parameters.length} parameters`;
    }

    if (filtered.length === 0) {
      paramList.innerHTML =
          '<p class="placeholder">No parameters match the filter criteria.</p>';
      return;
    }
    ...
    filtered.forEach(param => {
      ...
      typeBadge.textContent = param.type === 'input' ? 'Input' :
          param.type === 'lsm'                       ? 'LSM' :
                                                       'Holding';
      ...
      if (param.value !== undefined && param.value !== null &&
          param.value !== '' && !isNaN(numValue)) {
        valueCol.innerHTML = `<span class="value-dec">${
            numValue}</span><br><span class="value-hex">0x${
            numValue.toString(16).toUpperCase().padStart(4, '0')}</span>`;
      } else {
      ...
      statusBadge.className =
          `implemented-badge ${param.implemented === 'Y' ? 'yes' : 'no'}`;
    ...
    requestAnimationFrame(() => {
      paramList.querySelectorAll('.param-name-col[data-check-overflow="true"]')
          .forEach(nameCol => {
            const desc = nameCol.querySelector('.param-description');
            const name = nameCol.querySelector('.param-name');
            // clientWidth === 0 이면 탭이 숨겨진 상태 — 측정 불가이므로
            // expandable 부여하지 않음
            const descOverflow = desc && desc.clientWidth > 0 &&
                desc.scrollWidth > desc.clientWidth;
            const nameOverflow = name && name.clientWidth > 0 &&
      ...
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const param = {
            ...
            value: null,
            functionCode: values[headers.indexOf('type')] === 'input' ? 4 :
                values[headers.indexOf('type')] === 'lsm'             ? 0x2B :
                                                                        3
          };
        ...
        this.renderParameters();
        this.showToast(
            `${newParams.length}개의 파라미터를 가져왔습니다`, 'success');
      } catch (error) {
  ...
  exportCSV() {
    ...
    this.parameters.forEach(param => {
      const values = [
        param.type || 'holding', param.address || '',
        `"${(param.name || '').replace(/"/g, '""')}"`, param.implemented || 'N',
  ...
  loadDefaultCSV() {
    ...
    this.renderParameters();
    this.showToast(
        `기본 파라미터 ${newParams.length}개를 로드했습니다`, 'success');
  }
  ...
  getDefaultParameters() {
    return [
      // Holding Registers
      {
        ...
        name: 'Reset',
        implemented: 'Y',
        description: 'Software Reset, Error Reset, EEPROM to RAM'
      },
      {
        type: 'holding',
        group: 'Control',
        ...
        name: 'Setpoint',
        implemented: 'Y',
        description: '지령 값, 오픈루프 모드이면 % or 속도모드이면 RPM'
      },
      {
        type: 'holding',
        group: 'Control',
        address: '0xD00D',
        name: 'Stored set value',
        implemented: 'N',
        description:
            'Set Point 설정값이 EEPROM에 저장됨, 급작스러운 전원 공급 중단 및 복구 후 마지막 설정값으로 재시작'
      },
      {
        type: 'holding',
        group: 'Control',
        ...
        implemented: 'N',
        description:
            '서보드라이브의 SVON/SVOFF와 같은 기능, 0xD16A(Enable/Disable source)의 값에 따라 적절한 Enable state가 결정'
      },
      {
        type: 'holding',
        group: 'Information',
        address: '0xD005',
        name: 'Factory setting Control',
        implemented: 'N',
        description: '공장 초기값 변경 및 적용 용도'
      },
      {
        type: 'holding',
        group: 'Information',
        ...
        implemented: 'N',
        description: '65535시간까지 카운트 후 고정'
      },
      {
        type: 'holding',
        group: 'Information',
        address: '0xD00A',
        name: 'Operating minutes counter',
        implemented: 'N',
        description: '0분 to 59분 롤링'
      },
      {
        type: 'holding',
        group: 'Communication',
        ...
        name: 'Fan address',
        implemented: 'Y',
        description: 'Node ID와 같은 역할'
      },
      {
        type: 'holding',
        group: 'Communication',
        address: '0xD101',
        name: 'Set value source',
        implemented: 'Y',
        description:
            'Setpoint를 어떤 수단으로 사용할 것인지 설정 (0:AIN1, 1:RS485, 2:AIN2, 3:PWM)'
      },
      {
        type: 'holding',
        group: 'Communication',
        ...
        implemented: 'Y',
        description:
            'RS-485 통신 속도를 설정합니다. | 0:1200bps, 1:2400bps, 2:4800bps, 3:9600bps, 4:19200bps(default), 5:38400bps, 6:57600bps, 7:115200bps'
      },
      {
        type: 'holding',
        group: 'Communication',
        ...
        implemented: 'Y',
        description:
            '시리얼 통신의 데이터 비트, 패리티, 스톱 비트를 설정합니다. | 0:Data8/Even/Stop1(default), 1:Data8/Odd/Stop1, 2:Data8/None/Stop2, 3:Data8/None/Stop1'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        address: '0xD102',
        name: 'Preferred running direction',
        implemented: 'Y',
        description: '구동 방향 결정 (0:CCW, 1:CW)'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        ...
        implemented: 'Y',
        description:
            '모터 제어 방식을 설정합니다. | 0:Speed Control, 2:Open-loop control'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        address: '0xD112',
        name: 'Motor stop enable',
        implemented: 'N',
        description:
            '0: set value가 0이더라도 모터 항상 SVON, 1: set value가 0일 경우 모터 SVOFF'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        ...
        implemented: 'Y',
        description:
            '센서 제어모드 및 Open-loop control 모드에서는 이 파라미터에 지정된 속도로 제한 (토크모드에서의 속도제한 값)'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        address: '0xD11A',
        name: 'Maximum permissible speed',
        implemented: 'N',
        description: '최대 속도의 상한치를 설정 (모터 최대 속도)'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        ...
        implemented: 'Y',
        description:
            '가/감속도 조정 파라미터, 알람 등 모터 정지조건이 감지되면 감속없이 정지함'
      },
      {
        type: 'holding',
        group: 'Motor Control',
        address: '0xD120',
        name: 'Ramp-down curve',
        implemented: 'Y',
        description: '감속 곡선 설정'
      },
      {
        type: 'holding',
        group: 'Signal Mapping',
        address: '0xD12A',
        name: 'Point 1 X-coordinate',
        implemented: 'Y',
        description:
            '아날로그 입력 또는 PWM 입력 신호에 설정값을 할당하는데 사용'
      },
      {
        type: 'holding',
        group: 'Signal Mapping',
        address: '0xD12B',
        name: 'Point 1 Y-coordinate',
        implemented: 'Y',
        description:
            '아날로그 입력 또는 PWM 입력 신호에 설정값을 할당하는데 사용'
      },
      ...
      },
      {
        ...
        implemented: 'N',
        description:
            '0번 비트 set: Power limit 활성화, 1번 비트 set: Current Limit 활성화'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD135',
        name: 'Maximum permissible power',
        implemented: 'N',
        description: '허용 가능한 최대 파워 설정'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD136',
        name: 'Max. power at derating end',
        implemented: 'N',
        description: '모듈과 모터의 온도를 토대로 출력을 디레이팅 하는 기능'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD137',
        name: 'Module temperature power derating start',
        implemented: 'Y',
        description: '모듈 온도 파워 디레이팅 시작점'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD138',
        name: 'Module temperature power derating end',
        implemented: 'Y',
        description: '모듈 온도 파워 디레이팅 종료점'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD13B',
        name: 'Maximum coil current',
        implemented: 'Y',
        description:
            '전류 제한이 활성화 되면 모터 코일전류(rms값)를 이 파라미터에 설정된 값으로 제한'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD145',
        name: 'Speed limit for running monitoring',
        implemented: 'N',
        description:
            '실행 모니터링 속도제한, 속도 피드백이 이 파라미터에 설정된 속도보다 낮을 경우 오류 해제 (n_Low)'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD147',
        name: 'Sensor actual value source',
        implemented: 'Y',
        description: '지령으로 선택된 센서의 지령값'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD14D',
        name: 'Motor temperature power derating start address',
        implemented: 'N',
        description: '모터 온도 파워 디레이팅 시작 주소'
      },
      {
        type: 'holding',
        group: 'Limitation',
        address: '0xD14E',
        name: 'Motor temperature power derating end address',
        implemented: 'N',
        description: '모터 온도 파워 디레이팅 종료 주소'
      },
      {
        type: 'holding',
        group: 'Limitation',
        ...
        name: 'Maximum power',
        implemented: 'N',
        description: '최대 파워 설정'
      },
      {
        type: 'holding',
        group: 'I/O',
        address: '0xD158',
        name: 'Configuration of I/O 1',
        implemented: 'N',
        description: 'I/O 활성/비활성화 설정 파라미터'
      },
      {
        type: 'holding',
        group: 'I/O',
        address: '0xD159',
        name: 'Configuration of I/O 2',
        implemented: 'N',
        description: 'I/O 활성/비활성화 설정 파라미터'
      },
      {
      ...
      },
      {
        ...
        group: 'Sensor',
        address: '0xD160',
        name: 'Min. sensor value',
        implemented: 'Y',
        description: '입력 센서의 최소값'
      },
      {
        type: 'holding',
        group: 'Sensor',
        address: '0xD162',
        name: 'Max. sensor value',
        implemented: 'Y',
        description: '입력 센서의 최대값'
      },
      {
        type: 'holding',
        group: 'Shedding',
        address: '0xF150',
        name: 'Shedding function',
        implemented: 'N',
        description:
            '외부 환경에 의해 팬이 얼어 기동이 어려울 경우 이 기능 활성화를 통해 구속 상태를 제거'
      },
      {
        type: 'holding',
        group: 'Shedding',
        address: '0xF151',
        name: 'Max. starting modulation level',
        implemented: 'N',
        description: '최대 시작 모듈레이션 레벨'
      },
      {
        type: 'holding',
        group: 'Shedding',
        address: '0xF152',
        name: 'Number of start attempts',
        implemented: 'N',
        description: '시작 시도 횟수'
      },
      {
        type: 'holding',
        group: 'Shedding',
        address: '0xF153',
        name: 'Relay dropout delay',
        implemented: 'N',
        description:
            '에러나 경고 감지 시 릴레이 출력 지연시간을 설정하여 단기 이슈일 경우는 무시'
      },
      {
        type: 'holding',
        group: 'Customer Data',
        address: '0xD170',
        name: 'Customer data 0',
        implemented: 'N',
        description: '고객 사용 영역'
      },
      {
        type: 'holding',
        group: 'Customer Data',
        address: '0xD171',
        name: 'Customer data 1',
        implemented: 'N',
        description: '고객 사용 영역'
      },
      {
      ...
      },
      {
        ...
        group: 'Error / History',
        address: '0xD180',
        name: 'Operating hours counter (back-up)',
        implemented: 'N',
        description: '0xD009의 사용시간을 저장하는 파라미터'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '가장 최근 에러의 파라미터 번호를 표시'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        name: 'Error 1',
        implemented: 'N',
        description: '팬에서 발생한 첫 번째 오류 표시'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        name: 'Error time',
        implemented: 'N',
        description: '팬에서 발생한 에러를 최대 13개까지 저장'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 1'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD187',
        name: 'Error history time 1',
        implemented: 'N',
        description: '에러 히스토리 시간 1'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 2'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD189',
        name: 'Error history time 2',
        implemented: 'N',
        description: '에러 히스토리 시간 2'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 3'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD18B',
        name: 'Error history time 3',
        implemented: 'N',
        description: '에러 히스토리 시간 3'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 4'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD18D',
        name: 'Error history time 4',
        implemented: 'N',
        description: '에러 히스토리 시간 4'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 5'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD18F',
        name: 'Error history time 5',
        implemented: 'N',
        description: '에러 히스토리 시간 5'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 6'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD191',
        name: 'Error history time 6',
        implemented: 'N',
        description: '에러 히스토리 시간 6'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        implemented: 'N',
        description: '에러 히스토리 7'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD193',
        name: 'Error history time 7',
        implemented: 'N',
        description: '에러 히스토리 시간 7'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD194',
        name: 'Error history 8',
        implemented: 'N',
        description: '에러 히스토리 8'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD195',
        name: 'Error history time 8',
        implemented: 'N',
        description: '에러 히스토리 시간 8'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD196',
        name: 'Error history 9',
        implemented: 'N',
        description: '에러 히스토리 9'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD197',
        name: 'Error history time 9',
        implemented: 'N',
        description: '에러 히스토리 시간 9'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD198',
        name: 'Error history 10',
        implemented: 'N',
        description: '에러 히스토리 10'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD199',
        name: 'Error history time 10',
        implemented: 'N',
        description: '에러 히스토리 시간 10'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD19A',
        name: 'Error history 11',
        implemented: 'N',
        description: '에러 히스토리 11'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD19B',
        name: 'Error history time 11',
        implemented: 'N',
        description: '에러 히스토리 시간 11'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD19C',
        name: 'Error history 12',
        implemented: 'N',
        description: '에러 히스토리 12'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD19D',
        name: 'Error history time 12',
        implemented: 'N',
        description: '에러 히스토리 시간 12'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD19E',
        name: 'Error history 13',
        implemented: 'N',
        description: '에러 히스토리 13'
      },
      {
        type: 'holding',
        group: 'Error / History',
        address: '0xD19F',
        name: 'Error history time 13',
        implemented: 'N',
        description: '에러 히스토리 시간 13'
      },
      {
        type: 'holding',
        group: 'Error / History',
        ...
        name: 'Error Mask',
        implemented: 'N',
        description: '마스크 씌운 경고 또는 에러가 날 경우 릴레이 출력'
      },
      {
        type: 'holding',
        group: 'Error / History',
      ...
      },
      {
        ...
        group: 'Communication',
        address: '0xD1FF',
        name: 'Enable Termination resistor',
        implemented: 'Y',
        description: 'RS485 통신 종단 저항 설정'
      },
      // Input Registers
      {
        type: 'input',
      ...
      },
      {
        ...
        group: 'Device Info',
        address: '0xD001',
        name: 'Max. number of bytes',
        implemented: 'Y',
        description: '최대 바이트 수'
      },
      {
        type: 'input',
        group: 'Device Info',
        address: '0xD002',
        name: 'Bus controller software name',
        implemented: 'Y',
        description: 'Main 부트버전'
      },
      {
        type: 'input',
        group: 'Device Info',
        address: '0xD003',
        name: 'Bus controller software version',
        implemented: 'Y',
        description: 'Main 펌웨어 버전'
      },
      {
        type: 'input',
        group: 'Device Info',
        address: '0xD004',
        name: 'Commutation controller software name',
        implemented: 'Y',
        description: 'Inverter 부트 버전'
      },
      {
        type: 'input',
        group: 'Device Info',
        address: '0xD005',
        name: 'Commutation controller software version',
        implemented: 'Y',
        description: 'Inverter 펌웨어 버전'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD010',
        name: 'Actual speed (Relative)',
        implemented: 'Y',
        description: '상대 속도'
      },
      {
      ...
      },
      {
        ...
        implemented: 'Y',
        description: '전류 제한 레벨'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD01A',
        name: 'Current set value',
        implemented: 'N',
        description: '현재 설정값'
      },
      {
        ...
        group: 'Status',
        address: '0xD01B',
        name: 'Sensor actual value address',
        implemented: 'Y',
        description: '선택된 실제 센서 값'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD01C',
        name: 'Enable/Disable input state',
        implemented: 'N',
        description: 'Enable/Disable 입력 상태'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD023',
        name: 'Sensor actual value 1',
        implemented: 'Y',
        description: 'AIN1의 현재 측정값'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD024',
        name: 'Sensor actual value 2',
        implemented: 'Y',
        description: 'AIN2의 현재 측정값'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD025',
        name: 'Sensor actual value 3',
        implemented: 'Y',
        description: 'PWMIN3의 현재 측정 Duty값'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD026',
        name: 'Sensor actual value 4',
        implemented: 'Y',
        description: 'PWMIN3의 현재 측정 주파수값'
      },
      {
        type: 'input',
        group: 'Status',
        address: '0xD028',
        name: 'Current set value source',
        implemented: 'Y',
        description:
            '현재 사용 중인 소스 (0=AIN1 1=RS485 2=AIN2 3=PWMIn3 255=Fail-safe)'
      },
      {
        type: 'input',
        group: 'Electrical',
      ...
      },
      {
        ...
        implemented: 'Y',
        description: 'IGBT Temperature Sensor 값'
      },
      {
        type: 'input',
        group: 'Electrical',
        ...
        implemented: 'Y',
        description: '제어부 Temperature 값'
      },
      {
        type: 'input',
        group: 'Electrical',
        address: '0xD021',
        name: 'Current power (Relative)',
        implemented: 'N',
        description: '상대 전력'
      },
      {
      ...
      // FC 0x2B = 43 (decimal). 일반 Modbus FC03(holding) / FC04(input)과 다른
      // 접근 방식. 이 파라미터들을 읽고 쓸 때는 반드시 FC 0x2B 전용 프레임을
      // 사용해야 함. param.type === 'lsm' 조건으로 분기하여
      // readParameterByAddress() 등에서 처리 필요. 주소 공간: 0x2000~0x27FF
      // (일반 파라미터), 0x4000~0x4FFF (공장/고급 파라미터)
      // ============================================================
      // [Device Setup] 디바이스 식별 및 통신 설정
      {
        ...
        implemented: 'Y',
        description:
            '드라이브에 연결된 모터 종류를 식별하는 ID. 모터 교체 시 변경 필요'
      },
      {
        type: 'lsm',
        group: 'Device Setup',
        ...
        implemented: 'Y',
        description:
            'RS-485 버스 상의 슬레이브 주소 (Modbus Node ID와 동일 역할)'
      },
      // [Servo Tuning] 서보 제어 루프 게인 및 필터 튜닝
      {
        ...
        name: 'Inertia Ratio',
        implemented: 'Y',
        description: '부하 관성 대 모터 관성 비율. 서보 제어 응답성 튜닝에 사용'
      },
      {
        type: 'lsm',
        group: 'Servo Tuning',
        address: '0x2101',
        name: 'Position P Gain 1',
        implemented: 'Y',
        description: '위치 제어 루프 비례 게인 1'
      },
      {
        type: 'lsm',
        group: 'Servo Tuning',
        address: '0x2102',
        name: 'Velocity P Gain 1',
        implemented: 'Y',
        description: '속도 제어 루프 비례 게인 1'
      },
      {
        type: 'lsm',
        group: 'Servo Tuning',
        address: '0x2103',
        name: 'Velocity Time Constant 1',
        implemented: 'Y',
        description: '속도 루프 적분 시간 상수 1'
      },
      {
        type: 'lsm',
        group: 'Servo Tuning',
        address: '0x2104',
        name: 'Torque Command Filter TC 1',
        implemented: 'Y',
        description: '토크 지령 저역 통과 필터의 시간 상수 1'
      },
      // [Limitation] 토크 제한 및 드라이브 제어 입력
      {
        ...
        implemented: 'Y',
        description: '외부 입력 기반 양방향(CW) 토크 상한값'
      },
      {
        type: 'lsm',
        group: 'Limitation',
        ...
        implemented: 'Y',
        description: '외부 입력 기반 음방향(CCW) 토크 상한값'
      },
      {
        type: 'lsm',
        group: 'Limitation',
        address: '0x211F',
        name: 'Drive Control Input 1',
        implemented: 'Y',
        description: '드라이브 제어 입력 포트 1의 기능 할당 설정'
      },
      {
        type: 'lsm',
        group: 'Limitation',
        address: '0x2120',
        name: 'Drive Control Input 2',
        implemented: 'Y',
        description: '드라이브 제어 입력 포트 2의 기능 할당 설정'
      },
      // [Motion] 속도 지령 / 가감속 / 조그 운전
      {
        ...
        implemented: 'Y',
        description:
            '조그 운전 시 속도 지령값. 부호 있음(int16) - 양수: CW / 음수: CCW'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2301',
        name: 'Speed Accel Time',
        implemented: 'Y',
        description: '속도 지령 변화 시 가속 구간 시간'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2302',
        name: 'Speed Decel Time',
        implemented: 'Y',
        description: '속도 지령 변화 시 감속 구간 시간'
      },
      {
        type: 'lsm',
        group: 'Motion',
        ...
        implemented: 'Y',
        description:
            '가감속에 S커브 적용 시간. 0이면 선형 가감속, 값이 클수록 부드러운 가감속'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2304',
        name: 'Preset Jog Speed 0',
        implemented: 'Y',
        description: '프리셋 조그 속도 0. 부호 있음(int16) - 방향 포함'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2305',
        name: 'Preset Jog Speed 1',
        implemented: 'Y',
        description: '프리셋 조그 속도 1. 부호 있음(int16) - 방향 포함'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2306',
        name: 'Preset Jog Speed 2',
        implemented: 'Y',
        description: '프리셋 조그 속도 2. 부호 있음(int16) - 방향 포함'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2307',
        name: 'Preset Jog Speed 3',
        implemented: 'Y',
        description: '프리셋 조그 속도 3. 부호 있음(int16) - 방향 포함'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2308',
        name: 'Preset Jog Time 0',
        implemented: 'Y',
        description: 'Preset Jog Speed 0 운전 지속 시간'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x2309',
        name: 'Preset Jog Time 1',
        implemented: 'Y',
        description: 'Preset Jog Speed 1 운전 지속 시간'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x230A',
        name: 'Preset Jog Time 2',
        implemented: 'Y',
        description: 'Preset Jog Speed 2 운전 지속 시간'
      },
      {
        type: 'lsm',
        group: 'Motion',
        address: '0x230B',
        name: 'Preset Jog Time 3',
        implemented: 'Y',
        description: 'Preset Jog Speed 3 운전 지속 시간'
      },
      // [Device Info] 장치 정보 (읽기 전용)
      {
        ...
        implemented: 'Y',
        description: '드라이브 시리얼 번호 (ASCII 16자)'
      },
      {
        type: 'lsm',
        group: 'Device Info',
        address: '0x27F0',
        name: 'Main Boot Version',
        implemented: 'Y',
        description: '메인 MCU 부트로더 펌웨어 버전 (ASCII 7자)'
      },
      {
        type: 'lsm',
        group: 'Device Info',
        ...
        implemented: 'Y',
        description: '메인 MCU 애플리케이션 펌웨어 버전 (ASCII 7자)'
      },
      {
        type: 'lsm',
        group: 'Device Info',
        address: '0x27F2',
        name: 'Inverter Boot Version',
        implemented: 'Y',
        description: '인버터 MCU 부트로더 펌웨어 버전 (ASCII 7자)'
      },
      {
        type: 'lsm',
        group: 'Device Info',
        ...
        implemented: 'Y',
        description: '인버터 MCU 애플리케이션 펌웨어 버전 (ASCII 7자)'
      },
      // [Status] 상태 모니터링 (읽기 전용)
      {
        ...
        name: 'Feedback Speed',
        implemented: 'Y',
        description: '엔코더 기반 현재 모터 실제 속도 피드백 (부호 있음 int16)'
      },
      {
        type: 'lsm',
        group: 'Status',
        ...
        implemented: 'Y',
        description: '실내 온도 센서 1 측정값 (부호 있음 int16)'
      },
      {
        type: 'lsm',
        group: 'Status',
        ...
        implemented: 'Y',
        description: '실내 온도 센서 2 측정값 (부호 있음 int16)'
      },
      {
        type: 'lsm',
        group: 'Status',
        address: '0x2617',
        name: '7-Segment Display Data',
        implemented: 'Y',
        description: '드라이브 전면 7세그먼트에 표시 중인 내용 (ASCII 7자)'
      },
      {
        type: 'lsm',
        group: 'Status',
        address: '0x261A',
        name: 'Commanded Phase Angle',
        implemented: 'Y',
        description: '현재 지령 중인 모터 전기각 위상 (부호 있음 int16)'
      },
      // [Control] 프로시저 실행
      {
        type: 'lsm',
        ...
        implemented: 'Y',
        description:
            '특수 동작 명령 코드 (예: 원점 복귀, 초기화 등). Procedure Argument(0x2701)와 함께 사용'
      },
      {
        type: 'lsm',
        group: 'Control',
        address: '0x2701',
        name: 'Procedure Argument',
        implemented: 'Y',
        description: 'Procedure Code(0x2700) 실행 시 전달하는 인수값'
      },
      // ============================================================
      ...
      // [Aging] 에이징 운전 설정
      {
        ...
        group: 'Aging',
        address: '0x4004',
        name: 'Aging Overload Threshold',
        implemented: 'Y',
        description: '에이징 운전 중 과부하 판정 기준값'
      },
      {
        type: 'lsm',
        group: 'Aging',
      ...
      // [Factory] 공장 예약 파라미터 (용도 미공개)
      {
        ...
        group: 'Factory',
        address: '0x4017',
        name: 'Factory Reserved 4',
        implemented: 'Y',
        description: '공장 예약 파라미터 4 (용도 미공개)'
      },
      {
        type: 'lsm',
        group: 'Factory',
        address: '0x4018',
        name: 'Factory Reserved 5',
        implemented: 'Y',
        description: '공장 예약 파라미터 5 (용도 미공개)'
      },
      {
        type: 'lsm',
        group: 'Factory',
        address: '0x4019',
        name: 'Factory Reserved 6',
        implemented: 'Y',
        description: '공장 예약 파라미터 6 (용도 미공개)'
      },
      {
        type: 'lsm',
        group: 'Factory',
        address: '0x401A',
        name: 'Factory Reserved 7',
        implemented: 'Y',
        description: '공장 예약 파라미터 7 (용도 미공개)'
      },
      {
        type: 'lsm',
        group: 'Factory',
        address: '0x401B',
        name: 'Factory Reserved 8',
        implemented: 'Y',
        description: '공장 예약 파라미터 8 (용도 미공개)'
      },
      {
        type: 'lsm',
        group: 'Factory',
        address: '0x401C',
        name: 'Factory Reserved 9',
        implemented: 'Y',
        description: '공장 예약 파라미터 9 (용도 미공개)'
      },
      {
        type: 'lsm',
        group: 'Factory',
        address: '0x401D',
        name: 'Factory Reserved 10',
        implemented: 'Y',
        description: '공장 예약 파라미터 10 (용도 미공개)'
      }
    ];
  }
  ...
  async readParameterByAddress(param, silent = false) {
    ...
    // LSM: FC 0x2B (CANopen MEI Transport), address = object index, subIndex =
    // 0 input: FC 0x04 (Read Input Registers) holding: FC 0x03 (Read Holding
    // Registers)
    let frame;
    if (isLsm) {
    ...
    if (this.simulatorEnabled) {
      ...
      this.addMonitorEntry(
          'sent', frame, {functionCode, startAddress: address, quantity: 1});
      this.stats.requests++;
      this.updateStatsDisplay();
      ...
      if (isLsm) {
        ...
        if (!silent)
          this.showToast(
              `${param.name}: 시뮬레이터는 LSM(FC 0x2B)을 지원하지 않습니다`,
              'warning');
        return false;
      }
      ...
      if (response && response.length >= 5) {
        ...
        if (!silent)
          this.showToast(
              `${param.name}: ${value} (0x${value.toString(16).toUpperCase()})`,
              'success');
        return true;
      } else {
        this.updateStats(false);
        if (!silent)
          this.showToast(`${param.name} 읽기에 실패했습니다`, 'error');
        return false;
      }
    } else if (this.writer) {
      let value;
  ...
  toggleSimulator() {
    ...
    if (this.simulatorEnabled) {
      ...
      this.updateConnectionStatus(true);
      this.addMonitorEntry(
          'received', 'Virtual Modbus Slave Simulator activated');
      ...
      this.addMonitorEntry(
          'received', 'Virtual Modbus Slave Simulator deactivated');
  ...
  resetSimulator() {
    if (confirm('시뮬레이터의 모든 메모리를 초기화하시겠습니까?')) {
      ...
      this.addMonitorEntry(
          'received', 'Simulator memory reset to initial values');
    }
  }
  ...
  startSimulatorUI() {
    this.simulatorUpdateInterval = setInterval(() => {
      ...
      document.getElementById('simTemp').textContent =
          `${(state.holdingRegisters[0] / 100).toFixed(2)}°C`;
      document.getElementById('simHumid').textContent =
          `${(state.holdingRegisters[1] / 100).toFixed(2)}%`;
      document.getElementById('simPress').textContent =
          `${(state.holdingRegisters[2] / 10).toFixed(1)} hPa`;
      document.getElementById('simVolt').textContent =
          `${(state.holdingRegisters[3] / 10).toFixed(1)}V`;
      document.getElementById('simSpeed').textContent =
          state.holdingRegisters[5];

      document.getElementById('simCoil0').textContent =
          state.coils[0] ? 'ON' : 'OFF';
      document.getElementById('simCoil1').textContent =
          state.coils[1] ? 'ON' : 'OFF';
      document.getElementById('simCoil2').textContent =
          state.coils[2] ? 'ON' : 'OFF';

      document.getElementById('simCoil0').style.color =
          state.coils[0] ? '#28a745' : '#6c757d';
      document.getElementById('simCoil1').style.color =
          state.coils[1] ? '#dc3545' : '#6c757d';
      document.getElementById('simCoil2').style.color =
          state.coils[2] ? '#28a745' : '#6c757d';

      // Update displayed values (Modal)
      const modalSimTemp = document.getElementById('modal-simTemp');
      if (modalSimTemp) {
        modalSimTemp.textContent =
            `${(state.holdingRegisters[0] / 100).toFixed(2)}°C`;
        document.getElementById('modal-simHumid').textContent =
            `${(state.holdingRegisters[1] / 100).toFixed(2)}%`;
        document.getElementById('modal-simPress').textContent =
            `${(state.holdingRegisters[2] / 10).toFixed(1)} hPa`;
        document.getElementById('modal-simVolt').textContent =
            `${(state.holdingRegisters[3] / 10).toFixed(1)}V`;
        document.getElementById('modal-simSpeed').textContent =
            state.holdingRegisters[5];

        document.getElementById('modal-simCoil0').textContent =
            state.coils[0] ? 'ON' : 'OFF';
        document.getElementById('modal-simCoil1').textContent =
            state.coils[1] ? 'ON' : 'OFF';
        document.getElementById('modal-simCoil2').textContent =
            state.coils[2] ? 'ON' : 'OFF';

        document.getElementById('modal-simCoil0').style.color =
            state.coils[0] ? '#28a745' : '#6c757d';
        document.getElementById('modal-simCoil1').style.color =
            state.coils[1] ? '#dc3545' : '#6c757d';
        document.getElementById('modal-simCoil2').style.color =
            state.coils[2] ? '#28a745' : '#6c757d';
      }
    }, 500);
  }
  ...
  async sendModbusRequest() {
    ...
    if (!this.writer) {
      this.addMonitorEntry(
          'error',
          'Not connected to serial port. Enable simulator or connect to real device.');
      return;
    }
    ...
    try {
      ...
      if (functionCode === 43) {
        ...
        if (op === 'read') {
          const numData =
              parseInt(document.getElementById('fc2bNumData').value) || 2;
          frame = this.modbus.buildCANopenUpload(
              slaveId, index, subIndex, 0, numData);
        } else {
          const val = parseInt(
              document.getElementById('fc2bWriteValue').value || '0', 16);
          frame =
              this.modbus.buildCANopenDownload(slaveId, index, subIndex, val);
        }
        this.addMonitorEntry(
            'sent', frame,
            {functionCode: 0x2B, startAddress: (index << 8) | subIndex});
        this.addMonitorEntry(
            'error', 'FC 0x2B: 시뮬레이터 미지원 (실제 장치에서만 동작)');
        this.stats.requests++;
        this.updateStatsDisplay();
        return;
      }
  ...
  showToast(message, type = 'info', duration = 3000) {
    ...
    toast.innerHTML = `
            ...
            <div class="toast-timer-bar" style="animation-duration: ${
        duration}ms"></div>
        `;
    ...
    // Auto-remove when timer bar animation ends (respects hover-pause
  ...
  showProgressToast(message, total, onCancel = null) {
    ...
    const update = (current, successCount = current, errorCount = 0) => {
      ...
      ringFg.style.strokeDasharray =
          `${successLen.toFixed(2)} ${(circumference - successLen).toFixed(2)}`;
      ...
      ringErr.style.strokeDasharray =
          `${errorLen.toFixed(2)} ${(circumference - errorLen).toFixed(2)}`;
   ...
   * @returns {Promise<boolean>} - Resolves to true if user confirms, false if
   *     cancelled
   */
  ...
  showDeviceEditModal(deviceId, focusField = null) {
    ...
    configWrapper.style.cssText =
        'display: flex; flex-direction: column; width: 100%; min-height: 0; flex: 1;';

    const configContainer = document.createElement('div');
    configContainer.id = 'deviceSetupConfig';
    configContainer.style.cssText =
        'flex: 1; overflow-y: auto; display: block; min-height: 0;';
    ...
    // While the modal is open, the page's #deviceSetupConfig may contain child
    // elements with the same IDs (e.g. fanAddress_1, operatingMode_1). Because
    // the page container appears earlier in the DOM than the modal,
    // getElementById always finds the page's elements first, causing apply*
    // functions to read stale values instead of what the user typed in the
    // modal.
    //
    // Fix: clear the page container's content entirely while the modal is open
    // so that no duplicate IDs exist. Restore content + ID when the modal
    // closes.
  ...
  initDashboardUI() {
    ...
    const dashboardAutoScanBtn =
        document.getElementById('dashboardAutoScanBtn');
    if (dashboardAutoScanBtn) {
      dashboardAutoScanBtn.addEventListener(
          'click', () => this.startDeviceScan(true));
    }
    ...
    if (closeDeviceModalBtn) {
      closeDeviceModalBtn.addEventListener(
          'click', () => this.hideAddDeviceModal());
    }
    ...
    if (addDeviceModal) {
      ...
      addDeviceModal.addEventListener('click', (e) => {
        // Only close if both mousedown and click happened on the modal
        // background
        if (e.target.id === 'addDeviceModal' && mouseDownTarget &&
            mouseDownTarget.id === 'addDeviceModal') {
          this.hideAddDeviceModal();
        }
    ...
    if (deselectAllDevices) {
      deselectAllDevices.addEventListener(
          'click', () => this.deselectAllDevices());
    }
    ...
    if (allDeviceModeRpm && allDeviceModePct) {
      allDeviceModeRpm.addEventListener(
          'click', () => this.setAllDeviceMode(0));
      allDeviceModePct.addEventListener(
          'click', () => this.setAllDeviceMode(1));
    }
    ...
    document.querySelectorAll('#allDevicePresetBtns .preset-btn')
        .forEach(btn => {
          btn.addEventListener('click', () => {
            const percent = parseInt(btn.dataset.percent);
            const isRpm = document.getElementById('allDeviceModeRpm')
    ...
    if (allDeviceApplyBtn) {
      allDeviceApplyBtn.addEventListener(
          'click', () => this.applyAllDeviceSetpoint());
    }

    // All device Stop button
    const allDeviceStopBtn = document.getElementById('allDeviceStopBtn');
    if (allDeviceStopBtn) {
      allDeviceStopBtn.addEventListener(
          'click', () => this.stopSelectedDevices());
    }
    ...
    if (viewModeCard && viewModeList) {
      viewModeCard.addEventListener(
          'click', () => this.setDeviceViewMode('card'));
      viewModeList.addEventListener(
          'click', () => this.setDeviceViewMode('list'));
  ...
  updateSliderBackground(slider) {
    ...
    slider.style.background =
        `linear-gradient(to right, #007bff ${percent}%, #e9ecef ${percent}%)`;
  }
  ...
  updateSelectedDevicesChips() {
    ...
    const allDeviceControlSection =
        document.querySelector('.all-device-control-section');

    // Update selected count badge
    if (selectedCountEl) {
      selectedCountEl.textContent = `${this.selectedDevices.size}개 선택`;
      selectedCountEl.classList.toggle(
          'has-devices', this.selectedDevices.size > 0);
    }

    // Update section style based on selection, and collapse content when
    // nothing selected
    if (allDeviceControlSection) {
      const content =
          allDeviceControlSection.querySelector('.all-device-control-content');
    ...
    this.selectedDevices.forEach(deviceId => {
      ...
      if (device) {
        ...
        chip.innerHTML = `
                    ...
                    <span class="chip-remove" data-device-id="${
            deviceId}">&times;</span>
                `;
  ...
  saveDevice() {
    ...
    const operationMode =
        parseInt(document.getElementById('deviceOperationMode').value);
    ...
    if (slaveId !== 0) {
      ...
      if (existingDevice) {
        this.showToast(
            `Node ID ${slaveId}는 이미 사용 중입니다 (${existingDevice.name})`,
            'error');
        return;
      }
    }
    ...
    // Start auto polling if connection is active, on Dashboard, and wasn't
    // running
  ...
  async deleteDevice(deviceId) {
    ...
    if (confirmed) {
      ...
      if (this.currentPage === 'device-setup' &&
          this.currentSetupDeviceId === deviceId) {
  ...
  renderDeviceGrid() {
    ...
    const existingCards = grid.querySelectorAll(
        '.device-card, .device-list-item, .device-list-header');
  ...
  createDeviceListItem(device) {
    ...
    const statusInfo = this.getMotorStatusInfo(
        device.motorStatus, device.online || this.simulatorEnabled);
    ...
    item.innerHTML = `
        ...
        this.selectedDevices.has(device.id) ? 'checked' : ''}>
            <span class="device-name-group">
                <span class="device-name" title="Click to edit name">${
        device.name}</span>
                <span class="device-serial" data-serial-for="${device.id}">${
        ...
        device.slaveId === 0 ? 'unassigned' : ''}">
            ...
            <div class="device-value">
                <div class="device-value-number setpoint-display-value">
                    ${device.setpoint}<span class="device-value-unit">${
        modeText}</span>
                </div>
            </div>
            <div class="device-mode-btns compact">
                <button class="mode-btn ${
        device.operationMode === 0 ? 'active' : ''}" data-mode="0">RPM</button>
                <button class="mode-btn ${
        device.operationMode !== 0 ? 'active' : ''}" data-mode="2">%</button>
            </div>
            <div class="device-controls">
                <input type="number" class="device-setpoint-input" placeholder="${
        modeText}" min="0" max="${
        device.maxSpeed ||
        (device.operationMode === 0 ? 1600 : 100)}" value="${device.setpoint}">
  ...
  startEditDeviceName(deviceId, nameSpan) {
    ...
    const maxInputWidth = Math.max(
        150, Math.min(300, parentWidth - 20));  // Between 150px and 300px
    ...
    const saveEdit = () => {
      ...
      if (newName && newName !== currentName) {
        ...
        this.showToast(
            `장치 이름이 "${newName}"으로 변경되었습니다`, 'success');
    ...
    card.innerHTML = `
        ...
        device.slaveId === 0 ? 'ID 미할당' : 'ID: ' + device.slaveId}
            ...
            <div class="device-action-bar">
                <button class="btn-device-live-watch ${
        device.liveWatch !== false ? 'active' :
                                     ''}" title="Live Watch 켜기/끄기">
        ...
        device.actualSpeed}<span class="device-value-unit">RPM</span>
                ...
                <div class="device-controls">
                    <input type="number" class="device-setpoint-input" placeholder="Setpoint (${
        modeText})" min="0" max="${
        device.maxSpeed ||
        (device.operationMode === 0 ? 1600 : 100)}" value="${device.setpoint}">
                    <span class="input-unit">${modeText}</span>
        ...
        device.monitoringExpanded ? 'expanded' : ''}">
                    <span class="monitoring-toggle-icon">▶</span>
                    <span class="monitoring-title">Monitoring Parameters</span>
                    <span class="monitoring-count">(${
        (device.monitoringParams || []).length})</span>
                </div>
                <div class="monitoring-content" style="display: ${
        device.monitoringExpanded ? 'block' : 'none'};">
  ...
  addDragEventListeners(element) {
    element.addEventListener(
        'dragstart', (e) => this.handleDragStart(e, element));
  ...
  updateDeviceCardSelection(deviceId) {
    ...
    const element = document.querySelector(`.device-card[data-device-id="${
        deviceId}"], .device-list-item[data-device-id="${deviceId}"]`);
    if (element) {
      if (this.selectedDevices.has(deviceId)) {
  ...
  async applyDeviceSetpoint(deviceId, setpoint, silent = false) {
    ...
    try {
      ...
      await this.writeRegister(
          device.slaveId, this.REGISTERS.SETPOINT, rawSetpoint);
      ...
      if (!silent)
        this.showToast(
            `${device.name}: Setpoint ${setpoint}${unit} 적용되었습니다`,
            'success');
    } catch (error) {
      if (!silent)
        this.showToast(`${device.name}: 설정에 실패했습니다`, 'error');
    }
  }
  ...
  async performSoftwareReset(deviceId) {
    ...
    try {
      // Send software reset command: FC06, 0xD000, bit3(4번째 비트) set =
      // 0x0008
      await this.writeRegister(device.slaveId, 0xD000, 0x0008);
      this.showToast(
          `${device.name}: 소프트웨어 리셋이 완료되었습니다`, 'success');
  ...
  async readDeviceStatus(deviceId) {
    ...
    try {
      ...
      const status = await this.readInputRegisterWithTimeout(
          device.slaveId, this.REGISTERS.MOTOR_STATUS);
      device.motorStatus = status;

      // Read current setpoint
      const setpoint =
          await this.readRegister(device.slaveId, this.REGISTERS.SETPOINT);
      device.setpoint = setpoint;

      // Read operation mode
      const mode = await this.readRegister(
          device.slaveId, this.REGISTERS.OPERATION_MODE);
      device.operationMode = mode;

      // If Speed Control mode (RPM), read maximum speed
      if (mode === 0) {
        const maxSpeed = await this.readRegister(
            device.slaveId, this.REGISTERS.MAXIMUM_SPEED);
  ...
  /**
   * Initialize device mode by reading 0xD106 (Operating Mode) and 0xD119
   * (Maximum Speed) Called when a new device is added
   */
  async initializeDeviceMode(deviceId) {
    ...
    try {
      ...
      if (mode === null) {
        console.log(
            `initializeDeviceMode: Slave ${device.slaveId} no response`);
        return;
      }
      ...
      const modeText =
          mode === 0 ? 'Speed Control (RPM)' : 'Open-loop Control (%)';
      this.showToast(`${device.name}: ${modeText} 모드`, 'info');
    } catch (error) {
      console.log(
          `Failed to initialize device mode for ${device.name}:`, error);
      // Keep default values
    }
  }
  ...
  async changeDeviceMode(deviceId, newMode) {
    ...
    try {
      ...
      await this.writeRegister(
          device.slaveId, this.REGISTERS.OPERATION_MODE, newMode);
      ...
      const modeText =
          newMode === 0 ? 'Speed Control (RPM)' : 'Open-loop Control (%)';
      this.showToast(
          `${device.name}: ${modeText} 모드로 변경되었습니다`, 'success');
    } catch (error) {
  ...
  async applyAllDeviceSetpoint() {
    ...
    const setpoint =
        parseInt(document.getElementById('allDeviceSetpoint').value);
    const rpmBtn = document.getElementById('allDeviceModeRpm');
    ...
    this.showToast(
        `${successCount}개 장치에 Setpoint ${setpoint}${unit}이 적용되었습니다`,
        'success');
  }
  ...
  /**
   * Stop auto polling
   * @param {boolean} force - true: 연결 해제 등 강제 중단 (pendingResponse 즉시
   *     취소)
   *                          false(기본): 탭 전환 등 정상 중단 (in-flight
   * 사이클 자연 완료 대기)
   */
  stopAutoPolling(force = false) {
    this.autoPollingTimer = null;
    // isPolling은 pollNextDeviceSequential의 finally에서 관리 — 여기서 강제
    // 리셋하지 않음 (강제 리셋 시 in-flight TX가 끝나기 전에 refreshDevice가
    // 직접 TX를 쏴서 버스 충돌 발생)
    if (force) {
      this.isPolling = false;
      if (this.pendingResponse) {
    ...
    // 대기 중인 write 명령을 모두 reject (연결 해제 시 caller가 catch 처리
    // 가능)
  ...
  _syncAllDeviceLwBtns() {
    this.devices.forEach(d => {
      const card =
          document.querySelector(`.device-card[data-device-id="${d.id}"]`);
  ...
  updateLiveWatchToggleBtn() {
    ...
    btn.querySelector('.live-watch-label').textContent =
        active ? 'Live Watch ON' : 'Live Watch OFF';
  }
  ...
  scheduleNextPoll() {
    ...
    if (this.backgroundPollingEnabled && this.pollingWorker) {
      this.pollingWorker.postMessage(
          {command: 'start', interval: this.autoPollingInterval});
    } else {
      setTimeout(
          () => this.pollNextDeviceSequential(), this.autoPollingInterval);
    }
  }
  ...
  get _isFc64Active() {
    return this.chartRunning ||
        Object.values(this.miniChartRunning).some(v => v) || this._fc64Busy ||
        this.triggerRunning;
  }
  ...
  //  JS 싱글스레드 특성: while 루프 탈출 후 _busBusy = true 대입은 원자적으로
  //  실행됨
  // ─────────────────────────────────────────────────────────
  async _acquireBus() {
      ...
      try {
        if (cmd.type === 'read') {
          ...
          cmd.resolve(value);
            ...
            cmd.type === 'canopen_read' || cmd.type === 'canopen_write') {
          const result =
              await this.sendCANopenAndWaitResponse(cmd.frame, cmd.slaveId);
          cmd.resolve(result);
        } else {
          await this.sendWriteAndWaitResponse(
  ...
  async pollNextDeviceSequential() {
    ...
    if (this.commandQueue.length > 0) {
      ...
      try {
        ...
        this.isPolling = false;
        // Interval(설정의 Interval)만큼 대기 후 다음 사이클 (queue에 더 있으면
        // 즉시 처리)
        this.scheduleNextPoll();
      }
      return;
    ...
    const validDevices =
        this.devices.filter(d => d.slaveId !== 0 && d.liveWatch !== false);
    if (validDevices.length === 0) {
      this.isPolling = false;
      setTimeout(() => this.pollNextDeviceSequential(), 500);
      return;
    ...
    try {
      ...
      // Direct sendAndWaitResponse — bypasses queue since we are the polling
      // loop.
      const motorFrame = this.modbus.buildReadInputRegisters(
          device.slaveId, this.REGISTERS.MOTOR_STATUS, 1);
      ...
      if (status !== null) {
        ...
        const speedFrame = this.modbus.buildReadInputRegisters(
            device.slaveId, this.REGISTERS.ACTUAL_SPEED, 1);
        const actualSpeed =
            await this.sendAndWaitResponse(speedFrame, device.slaveId);
        if (actualSpeed !== null) {
          // Handle signed 16-bit value
          device.actualSpeed =
              actualSpeed > 32767 ? actualSpeed - 65536 : actualSpeed;
        }
        ...
        const setpointFrame = this.modbus.buildReadHoldingRegisters(
            device.slaveId, this.REGISTERS.SETPOINT, 1);
        const rawSetpoint =
            await this.sendAndWaitResponse(setpointFrame, device.slaveId);
        if (rawSetpoint !== null) {
        ...
        // Inter-frame gap before monitoring params (FC03→FC04 전환 시 Modbus
        // RTU 3.5 char 간격 보장)
        if (this.paramPollingDelay > 0)
          await this.delay(this.paramPollingDelay);
    ...
    if (this.simulatorEnabled) {
      return new Promise(async (resolve) => {
        ...
        try {
          const response = await this.simulator.processRequest(frame);
          if (response && response.length >= 5 &&
              response[0] === expectedSlaveId && (response[1] & 0x80) === 0) {
            this.addMonitorEntry('received', response);
            this.stats.success++;
            this.updateStatsDisplay();
    ...
    try {
      return await new Promise(async (resolve) => {
        ...
        try {
          ...
          this.addMonitorEntry(
              'error',
              `Slave ${expectedSlaveId}: Response timeout (${
                  this.pollingTimeout}ms)`);
          resolve(null);
        }
      });
    } finally {
   ...
   * @returns {Object} { text: string, class: string, errors: array, hasError:
   *     boolean }
   */
  getMotorStatusInfo(status, online = true) {
    ...
    return {
      ...
      errors: [],
      hasError: true,
      tooltip: `Alarm — undefined bit (0x${
          status.toString(16).toUpperCase().padStart(4, '0')})`
    };
  }
  ...
  async writeRegister(slaveId, address, value) {
    ...
    if (this.simulatorEnabled) {
      this.addMonitorEntry(
          'sent', frame, {functionCode: 6, startAddress: address});
      this.stats.requests++;
      this.updateStatsDisplay();
      const response = await this.simulator.processRequest(frame);
      if (response) {
    ...
    try {
      await new Promise(async (resolve, reject) => {
        ...
        try {
          ...
          if (response && response.length >= 5 && response[0] === slaveId &&
              (response[1] & 0x80) === 0) {
            this.stats.success++;
            this.updateStatsDisplay();
            resolve();
    ...
    try {
      return await new Promise(async (resolve) => {
        ...
        try {
          ...
          this.addMonitorEntry(
              'sent', frame,
              {functionCode: 0x2B, startAddress: (frame[6] << 8) | frame[7]});
          this.stats.requests++;
          this.updateStatsDisplay();
          ...
          this.addMonitorEntry(
              'error',
              `Slave ${slaveId}: CANopen response timeout (${
                  this.commandTimeout}ms)`);
          resolve(null);
        }
      });
  ...
  /**
   * Read register helper (simplified - real implementation would wait for
   * response)
   */
  async readRegister(slaveId, address) {
    ...
    if (this.simulatorEnabled) {
      this.addMonitorEntry(
          'sent', frame, {functionCode: 3, startAddress: address, quantity: 1});
      this.stats.requests++;
      this.updateStatsDisplay();
      const response = await this.simulator.processRequest(frame);
      if (response && response.length >= 5) {
      ...
      // Delegate to readRegisterWithTimeout which handles queue + response
      // parsing correctly
      return await this.readRegisterWithTimeout(slaveId, address);
    } else {
      throw new Error('Not connected');
  ...
  generateParameterOptions() {
    ...
    if (holdingParams.length > 0) {
      ...
      holdingParams.forEach(p => {
        optionsHtml += `<option value="holding:${p.address}">${p.name} (${
            p.address})</option>`;
      });
    ...
    if (inputParams.length > 0) {
      ...
      inputParams.forEach(p => {
        optionsHtml += `<option value="input:${p.address}">${p.name} (${
            p.address})</option>`;
      });
      optionsHtml += '</optgroup>';
    }
  ...
  generateParamPickerHTML() {
    ...
    const grouped = {};
    this.parameters.forEach(p => {
      const typeLabel = p.type === 'input' ? 'Input' :
          p.type === 'lsm'                 ? 'LSM' :
                                             'Holding';
      const groupKey = `${typeLabel} — ${
          p.group ||
          (p.type === 'input'   ? 'Input Registers' :
               p.type === 'lsm' ? 'LSM' :
                                  'Holding Registers')}`;
    ...
    keys.forEach((groupKey, i) => {
      catHtml += `<div class="param-picker-cat-item${
          i === 0 ? ' active' :
                    ''}" data-group="${groupKey}">${groupKey}</div>`;
      grouped[groupKey].forEach(p => {
        paramHtml += `<div class="param-picker-item${
            i === 0 ? '' : ' hidden-group'}" data-value="${p.type}:${
            p.address}" data-group="${groupKey}">${
            p.name}<span class="param-picker-addr">${p.address}</span></div>`;
      });
    });
  ...
  getChartChannels() {
    return [
      ...
      {
        name: 'Reserved 3 (L.Sensor Valid Raw)',
        chNum: 0x2B,
        group: 'Sensor/LMS'
      },
      {
        name: 'Reserved 4 (R.Sensor Valid Raw)',
        chNum: 0x2C,
        group: 'Sensor/LMS'
      },
      // FFT
  ...
  generateChannelPickerHTML() {
    ...
    keys.forEach((group, i) => {
      catHtml += `<div class="param-picker-cat-item${
          i === 0 ? ' active' : ''}" data-group="${group}">${group}</div>`;
      grouped[group].forEach(ch => {
        const hex = `0x${ch.chNum.toString(16).toUpperCase().padStart(2, '0')}`;
        itemHtml += `<div class="param-picker-item${
            i === 0 ? '' : ' hidden-group'}" data-value="${
            ch.chNum}" data-group="${group}">${
            ch.name}<span class="param-picker-addr">${hex}</span></div>`;
      });
    });
    return `
            <div class="param-picker-panels">
  ...
  initChartChannelPickers() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      // Category click
      popup.querySelector('.param-picker-cat-panel')
          .addEventListener('click', (e) => {
            const cat = e.target.closest('.param-picker-cat-item');
            if (!cat) return;
            popup.querySelectorAll('.param-picker-cat-item')
                .forEach(el => el.classList.remove('active'));
            cat.classList.add('active');
            const group = cat.dataset.group;
            popup.querySelectorAll('.param-picker-item').forEach(item => {
              item.classList.toggle(
                  'hidden-group', item.dataset.group !== group);
            });
          });

      // Item click
      popup.querySelector('.param-picker-param-panel')
          .addEventListener('click', (e) => {
            ...
            triggerBtn.querySelector('.param-picker-trigger-label')
                .textContent = `${name} (${hex})`;
      ...
      searchInput.addEventListener('input', () => {
        ...
        if (q) {
          ...
          popup.querySelectorAll('.param-picker-item').forEach(item => {
            item.classList.toggle(
                'hidden-group', !item.textContent.toLowerCase().includes(q));
          });
        } else {
          catPanelEl.style.display = '';
          const activeCat =
              catPanelEl.querySelector('.param-picker-cat-item.active');
          const group = activeCat ? activeCat.dataset.group : null;
          popup.querySelectorAll('.param-picker-item').forEach(item => {
            item.classList.toggle(
                'hidden-group', group && item.dataset.group !== group);
          });
        }
      });
      ...
      triggerBtn.addEventListener('click', (e) => {
        ...
        document.querySelectorAll('.param-picker-popup').forEach(p => {
      ...
      if (defCh) {
        ...
        triggerBtn.querySelector('.param-picker-trigger-label').textContent =
            `${defCh.name} (${hex})`;
        triggerBtn.classList.add('has-selection');
      }
  ...
  initTriggerSourcePicker() {
    ...
    catPanel.querySelectorAll('.param-picker-cat-item:not(:first-child)')
        .forEach(el => el.classList.remove('active'));
    ...
    immItem.innerHTML =
        'Immediate Trigger<span class="param-picker-addr">0xFF</span>';
    paramPanel.insertBefore(immItem, paramPanel.firstChild);
    // 기존 아이템들은 Immediate 카테고리가 아니므로 hidden
    paramPanel.querySelectorAll('.param-picker-item:not(:first-child)')
        .forEach(el => el.classList.add('hidden-group'));
  ...
  addMonitoringParamFromCSV(deviceId, paramValue) {
    ...
    if (device.monitoringParams.some(
            p => p.type === type && p.address === address)) {
      this.showToast('이미 존재하는 파라미터입니다', 'warning');
      return;
    }

    // Find name from parameters
    const csvParam = this.parameters.find(
        p => p.type === type &&
            this.parseMonitoringAddress(p.address) === address);
        ...
        params
            ...
            <div class="monitoring-param-item" data-param-id="${param.id}">
                <div class="param-info">
                    ...
                    <span class="param-address">${
                    param.type === 'holding' ? 'H' : 'I'}:0x${
                    param.address.toString(16).toUpperCase()}</span>
                </div>
                <div class="param-value-wrapper">
                    <div class="param-value ${
                    param.value === null ? 'stale' :
                                           ''}" id="param-value-${param.id}">
                        ${param.value !== null ? param.value : '--'}
                    </div>
                    ${this.getParamConvertedHTML(param)}
                </div>
                <button class="param-remove-btn" data-device-id="${
                    deviceId}" data-param-id="${
                    param.id}" title="Remove">×</button>
            </div>
        `).join('');
    ...
    return params
        .map(
            ...
            <div class="monitoring-param-item" data-param-id="${param.id}">
                ...
                param.type === 'holding' ?
                    'H' :
                    'I'}:0x${param.address.toString(16).toUpperCase()}</span>
                </div>
                <div class="param-value-wrapper">
                    <div class="param-value ${
                param.value === null ? 'stale' :
  ...
  setupMonitoringEventListeners(card, deviceId) {
    ...
    if (header) {
      header.addEventListener(
          'click', () => this.toggleMonitoringSection(deviceId));
    }
    ...
    if (triggerBtn) {
      ...
      document
          .querySelectorAll(`.param-picker-popup[data-device-id="${deviceId}"]`)
          .forEach(p => p.remove());
    ...
    if (manualAddrInput && manualAddrConverted) {
      manualAddrInput.addEventListener('input', () => {
        this.updateConvertedValueEl(
            manualAddrInput, manualAddrConverted, 0, 65535);
      });
    }
    ...
    for (const param of device.monitoringParams) {
      try {
        ...
        if (param.type === 'lsm') {
          ...
          const paramFrame =
              this.modbus.buildCANopenUpload(device.slaveId, param.address, 0);
          const result =
              await this.sendCANopenAndWaitResponse(paramFrame, device.slaveId);
          value = result?.value ?? null;
        } else {
          // Direct sendAndWaitResponse — bypasses queue since we are inside the
          // polling loop.
          const paramFrame = param.type === 'input' ?
              this.modbus.buildReadInputRegisters(
                  device.slaveId, param.address, 1) :
              this.modbus.buildReadHoldingRegisters(
                  device.slaveId, param.address, 1);
        ...
        console.warn(
            `Failed to read param ${param.name} for device ${device.id}:`,
            error);
      }
  ...
  /**
   * Returns converted sub-value HTML for special registers (D023 → mV, D024 →
   * mA)
   */
  ...
  initSerialPortScanUI() {
    ...
    if (startBtn)
      startBtn.addEventListener('click', () => this.startSerialPortScan());
    if (stopBtn)
      stopBtn.addEventListener('click', () => this.stopSerialPortScan());
  }

  /**
   * Scan through serial port setting combinations (baudRate × parity ×
   * stopBits) and run device scan on each combination.
   */
  async startSerialPortScan() {
    ...
    const baudRates =
        [...document.querySelectorAll('.spScan-baudRate:checked')].map(
            e => parseInt(e.value));
    const parities =
        [...document.querySelectorAll('.spScan-parity:checked')].map(
            e => e.value);
    const stopBitOpts =
        [...document.querySelectorAll('.spScan-stopBits:checked')].map(
            e => parseInt(e.value));
    ...
    const origBaud =
        parseInt(document.getElementById('sidebar-baudRate')?.value) || 19200;
    const origParity =
        document.getElementById('sidebar-parity')?.value || 'even';
    const origStop =
        parseInt(document.getElementById('sidebar-stopBits')?.value) || 1;
    ...
    for (let i = 0; i < combos.length; i++) {
      ...
      if (progressText)
        progressText.textContent = `(${i + 1}/${combos.length}) ${
            baudRate} bps · ${parity} · Stop ${stopBits} — 스캔 중...`;

      try {
        await this.reconnectSerial(
            baudRate, parity, stopBits, true /* silent */);
        ...
        allResults.push(
            {baudRate, parity, stopBits, foundIds: [], error: e.message});
      }
    }
    ...
    if (progressText)
      progressText.textContent = this.scanAborted ?
    ...
    if (resultsDiv && resultsList) {
      ...
      if (allResults.length === 0 || successResults.length === 0) {
        resultsList.innerHTML =
            '<p style="color:#6c757d;">어떤 설정에서도 장치를 찾지 못했습니다.</p>';
      } else {
        resultsList.innerHTML =
            allResults
                .map(r => {
                  const label =
                      `${r.baudRate} bps · ${r.parity} · Stop ${r.stopBits}`;
                  if (r.error) {
                  ...
                  return `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px;margin-bottom:5px;border-radius:4px;background:white;border:1px solid #c3e6cb;">
                      ...
                      label}</strong><br>
                            <span style="font-size:12px;color:#6c757d;">발견된 장치: ${
                      idList}</span>
                        </div>
                        <button class="btn btn-sm btn-primary spScan-applyBtn"
                            data-baud="${r.baudRate}" data-parity="${
                      r.parity}" data-stop="${r.stopBits}">
                            이 설정 적용
                        </button>
                    </div>`;
                })
                .filter(Boolean)
                .join('') ||
            '<p style="color:#6c757d;">장치를 발견한 설정이 없습니다.</p>';
  ...
  /**
   * Internal: scan all slave IDs in the configured range, return array of found
   * IDs. Does NOT update the device list or show scan UI.
   */
  async _scanAllSlaveIds() {
    this.isScanning = true;
    const found = [];
    for (let slaveId = this.scanRangeStart; slaveId <= this.scanRangeEnd;
         slaveId++) {
      if (this.scanAborted) break;
   ...
   * @param {boolean} autoAdd - If true, automatically add found devices to
   *     Dashboard
   */
  async startDeviceScan(autoAdd = false) {
    ...
    // 스캔 시작 전 이미 등록된 디바이스 ID 저장 (스캔 완료 후 operationMode
    // 동기화용)
    const preExistingDeviceIds =
        this.devices.filter(d => d.slaveId !== 0).map(d => d.id);
    ...
    const foundDevices = [];
    const respondedSlaveIds =
        new Set();  // 응답한 모든 slaveId (신규 + 기존 포함)
    ...
    const scanToast = this.showProgressToast(
        ...
        totalToScan, () => this.stopDeviceScan());

    this.addMonitorEntry(
        'received',
        `Device scan started (ID ${this.scanRangeStart} ~ ${
            this.scanRangeEnd})${autoAdd ? ' [Auto Add]' : ''}`);
    ...
    for (let slaveId = this.scanRangeStart; slaveId <= this.scanRangeEnd;
      ...
      if (scanProgressText)
        scanProgressText.textContent =
            `Scanning ID ${slaveId}... (${current}/${totalToScan})`;
      ...
      if (response !== null) {
        ...
        if (!alreadyExists) {
          foundDevices.push(
              {slaveId: slaveId, responseValue: response, isNew: true});
          this.addMonitorEntry(
              'received', `New device found at Slave ID ${slaveId}`);
    ...
    if (scanResults && scanResultsList) {
      ...
      if (foundDevices.length === 0) {
        scanResultsList.innerHTML =
            '<p style="color: #6c757d;">발견된 장치가 없습니다.</p>';
      } else {
        scanResultsList.innerHTML =
            foundDevices
                .map(d => {
                  const alreadyAdded = autoAdd ||
                      this.devices.some(dev => dev.slaveId === d.slaveId);
                  return `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: white; margin-bottom: 5px; border-radius: 4px; border: 1px solid #e0e6ed;">
                        <span><strong>ID ${d.slaveId}</strong> - Status: ${
                      d.responseValue}</span>
                        <button class="btn btn-sm btn-success add-scanned-device" data-slave-id="${
                      d.slaveId}" ${alreadyAdded ? 'disabled' : ''}>${
                      alreadyAdded ? '추가됨' : '추가'}</button>
                    </div>
                `;
                })
                .join('');
        ...
        if (!autoAdd) {
          scanResultsList
              .querySelectorAll('.add-scanned-device:not([disabled])')
              .forEach(btn => {
                btn.addEventListener('click', () => {
    ...
    const autoAddMsg =
        autoAdd && foundDevices.length > 0 ? ' (자동 추가됨)' : '';
    ...
    if (this.scanRemoveNotFound && !this.scanAborted) {
      const toRemove = this.devices.filter(
          dev => dev.slaveId >= this.scanRangeStart &&
      ...
      if (toRemove.length > 0) {
        ...
        this.renderDeviceGrid();
        this.showToast(
            `미응답 디바이스 ${toRemove.length}개가 삭제되었습니다`, 'info');
      }
    }

    // 스캔 완료 후 Device 탭이 열려있으면 파라미터 자동 읽기
    // readRegisterWithTimeout / readParameterByAddress 는 내부에서 큐
    // 처리하므로 폴링 재개 이후 안전하게 순차 실행됨
    this._autoReadAfterScan();

    // 스캔 전부터 존재했던 디바이스의 operationMode 동기화
    // (신규 발견 디바이스는 addScannedDevice() → initializeDeviceMode()로 이미
    // 처리됨)
  ...
  /**
   * 스캔 완료 직후 Configuration / Parameters 탭이 활성화된 경우 자동으로 값을
   * 읽어옴. 폴링이 재개될 시간을 확보하기 위해 300ms 지연 후 실행.
   */
  _autoReadAfterScan() {
    if (this.currentPage !== 'device-setup') return;

    const activeTab =
        sessionStorage.getItem('deviceSetupTab') || 'configuration';
  ...
  async scanSlaveId(slaveId) {
    try {
      ...
      const frame =
          this.modbus.buildReadInputRegisters(slaveId, scanAddress, 1);
      ...
      if (this.simulatorEnabled) {
        ...
        if (simulatedDevices.includes(slaveId)) {
          ...
          this.addMonitorEntry(
              'sent', frame,
              {functionCode: 4, startAddress: scanAddress, quantity: 1});
          this.stats.requests++;
          this.updateStatsDisplay();
  ...
  async scanWithTimeout(frame, slaveId) {
    ...
    // cycle that was already executing (e.g. waiting for Slave 7's response)
    // must complete first — otherwise the scan TX collides with the pending
    // poll response.
  ...
  addScannedDevice(slaveId, silent = false) {
    ...
    if (exists) {
      if (!silent) {
        this.showToast(
            `ID ${slaveId} 장치가 이미 등록되어 있습니다`, 'warning');
      }
      return false;
    }
  ...
  initFirmwareSourceSelector() {
    ...
    const switchSource = (source) => {
      ...
      document.getElementById('fwPanelLocal').style.display =
          source === 'local' ? 'block' : 'none';
      document.getElementById('fwPanelOnline').style.display =
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener(
          'click', () => this.loadFirmwareVersionList());
    }
        ...
        versions
            .map((v, idx) => {
              ...
              return `
                <tr class="fw-version-main-row" data-version-idx="${idx}">
                    <td>
                        <label class="fw-radio-label">
                            <input type="radio" name="fwVersionRadio" class="fw-version-radio" value="${
                  idx}">
                            <span class="fw-version-number">${
                  v.version}</span>${latestBadge}
                        </label>
                    </td>
                ...
                <tr class="fw-changelog-row" id="fwChangelogRow${
                  idx}" style="display:none;">
                    <td colspan="3" class="fw-changelog-detail-cell">
                        <div class="fw-changelog-detail-inner">
                            ...
                            <div class="fw-changelog-content">${
                  changelog.replace(/\n/g, '<br>')}</div>
                        </div>
                    </td>
                </tr>
    ...
    tbody.querySelectorAll('.fw-version-radio').forEach(radio => {
      radio.addEventListener('change', () => {
        if (radio.checked) {
          ...
          tbody.querySelectorAll('.fw-version-main-row')
              .forEach(tr => tr.classList.remove('fw-version-row-selected'));
  ...
  async selectOnlineFirmwareVersion(version) {
    ...
    try {
      ...
      this.firmwareFile = {
        ...
        size: this.firmwareData.length,
        _onlineVersion: version.version
      };
      ...
      if (selectedSize)
        selectedSize.textContent =
            this.formatFileSize(this.firmwareData.length);

      this.updateFirmwareButtons();
      this.showToast(
          `${version.version} 펌웨어 로드가 완료되었습니다`, 'success');
  ...
  _clearOnlineSelection() {
    ...
    document.querySelectorAll('.fw-version-main-row')
        .forEach(tr => tr.classList.remove('fw-version-row-selected'));
    document.querySelectorAll('.fw-version-radio')
        .forEach(r => r.checked = false);
    document.querySelectorAll('.fw-changelog-row')
        .forEach(r => r.style.display = 'none');
    document.querySelectorAll('.fw-changelog-arrow').forEach(a => {
      a.textContent = '▼';
      a.classList.remove('fw-arrow-open');
    });
    this.updateFirmwareButtons();
  ...
  handleFirmwareFile(file) {
    ...
    if (!isValid) {
      this.showToast(
          '지원하지 않는 파일 형식입니다. (.bin, .hex, .fw)', 'error');
      return;
    }
  ...
  verifyFirmware() {
    ...
    this.addFirmwareLog(
        `체크섬: 0x${checksum.toString(16).toUpperCase().padStart(4, '0')}`,
        'success');
  ...
  async startFirmwareDownload() {
    ...
    if (!this.isConnected && !this.simulatorEnabled) {
      this.showToast(
          '시리얼 포트에 연결하거나 시뮬레이터를 활성화하세요', 'error');
      return;
    }
    ...
    try {
      ...
      const initResponse = await this.sendAndReceive(
          initFrame, responseTimeout, {minLength: 1, skipCRC: true});
      if (!initResponse) {
      ...
      // 응답: 65바이트, CRC 없음
      const eraseResponse = await this.sendAndReceive(
          eraseFrame, responseTimeout, {minLength: 65, skipCRC: true});
      if (!eraseResponse) {
        throw new Error('Erase 확인 응답 없음 (timeout)');
      }
      this.addFirmwareLog(
          `RX: ${this.modbus.bufferToHex(eraseResponse)}`, 'rx');

      // Parse erase response
      const eraseResult = this.modbus.parseFirmwareResponse(eraseResponse);
      if (!eraseResult.success) {
        const statusHex =
            eraseResult.data?.eraseStatus?.toString(16).toUpperCase() ||
            'unknown';
      ...
      this.addFirmwareLog(`[0x03] 데이터 전송 시작 - 패킷 크기: ${
          packetSize} bytes, 총 ${totalPackets} 패킷`);
      ...
      while (transferred < totalSize) {
        ...
        if (packetCount % 10 === 0 || chunkSize < packetSize) {
          this.addFirmwareLog(
              `TX[${packetCount}]: ${chunkSize} bytes @ offset ${transferred}`,
              'tx');
        }

        // 응답: 65바이트, CRC 없음
        const dataResponse = await this.sendAndReceive(
            dataFrame, responseTimeout, {minLength: 65, skipCRC: true});
        if (!dataResponse) {
        ...
        if (!dataResult.success) {
          throw new Error(
              `데이터 전송 에러 (packet ${packetCount}): OpCode 0x05`);
        }
        ...
        if (dataResult.data?.totalReceivedByte !== expectedTotal) {
          this.addFirmwareLog(
              `경고: Total Received Byte 불일치 (expected: ${
                  expectedTotal}, got: ${dataResult.data?.totalReceivedByte})`,
              'warning');
        }
        ...
        if (progressStatus)
          progressStatus.textContent = `${this.formatFileSize(transferred)} / ${
              this.formatFileSize(totalSize)}`;

        // Update step progress with detailed info
        this.updateFirmwareDataProgress(
            transferred, totalSize, packetCount, totalPackets, estimatedTimeMs);
      }

      this.setFirmwareStepStatus('0x03', 'completed');
      this.updateFirmwareDataProgress(
          totalSize, totalSize, totalPackets, totalPackets, 0);
      this.addFirmwareLog(
          `[0x03] 데이터 전송 완료 - ${packetCount} 패킷, ${totalSize} bytes`,
          'success');
      ...
      // 응답: 65바이트, CRC 없음
      const doneResponse = await this.sendAndReceive(
          doneFrame, responseTimeout, {minLength: 65, skipCRC: true});
      if (!doneResponse) {
      ...
      this.addFirmwareLog(
          '[0x99] 펌웨어 업데이트 완료 - Flash Lock', 'success');
      ...
      this.showToast(
          `펌웨어 다운로드에 실패했습니다: ${error.message}`, 'error');
   ...
   */
  updateFirmwareDataProgress(
      transferred, total, currentPacket = 0, totalPackets = 0,
      remainingTimeMs = 0) {
    const progressFill = document.getElementById('fwDataProgressFill');
    const progressPercent = document.getElementById('fwDataProgressPercent');
    const progressBytes = document.getElementById('fwDataProgressBytes');
    ...
    if (progressTime) {
      if (remainingTimeMs > 0) {
        progressTime.textContent =
            `남은 시간: ${this.formatTime(remainingTimeMs)}`;
   ...
   * @param {object} options - Options: { minLength: minimum response bytes,
   *     skipCRC: skip CRC verification }
   */
  ...
  initializeChartPage() {
    ...
    // sampleRateEl: FC 0x64 Period 드롭다운 — startChartCapture() 시점에 읽음,
    // 리스너 불필요
    ...
    if (exportCsvBtn) {
      exportCsvBtn.addEventListener(
          'click', () => this.chartManager.exportToCSV());
    }

    if (exportPngBtn) {
      exportPngBtn.addEventListener(
          'click', () => this.chartManager.exportToPNG());
    }
    ...
    const setYAxisModeUI = (mode) => {
      [yAxisModeA, yAxisModeB, yAxisModeC].forEach(
          btn => btn?.classList.remove('active'));
      if (mode === 'independent')
        yAxisModeA?.classList.add('active');
      else if (mode === 'normalize')
        yAxisModeB?.classList.add('active');
      else
        yAxisModeC?.classList.add('active');
      scaleOffsetDivs.forEach(d => {
        d.style.display = mode === 'scaleoffset' ? 'flex' : 'none';
      });
    ...
    if (yAxisModeA)
      yAxisModeA.addEventListener('click', () => setYAxisModeUI('independent'));
    if (yAxisModeB)
      yAxisModeB.addEventListener('click', () => setYAxisModeUI('normalize'));
    if (yAxisModeC)
      yAxisModeC.addEventListener('click', () => setYAxisModeUI('scaleoffset'));
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (offsetEl) {
        offsetEl.addEventListener('change', () => {
          this.chartManager.setChannelOffset(
              i, parseFloat(offsetEl.value) || 0);
        });
      }
    }
  ...
  /**
   * Start chart data capture — Continuous(FC 0x64) 또는 Trigger(FC 0x65) 모드
   * 분기
   */
  ...
  async chartDataLoop() {
    ...
    while (this.chartRunning) {
      ...
      if (parsed.data.length > 0) {
        ...
        // 채널 우선(Channel-first) 순서: [CH1_s0, CH1_s1, ..., CH2_s0, CH2_s1,
        // ..., CH3_s0, ...] 타임스탬프를 패킷 도착 시점이 아닌 누적 카운터로
        // 할당 → Len 변동 시에도 연속성 보장
        for (let s = 0; s < samplesPerCh; s++) {
          ...
          this.chartConfiguredChannels.forEach((ch, cfgIdx) => {
            ...
            if (s === samplesPerCh - 1) {
              const valueEl =
                  document.getElementById(`chartCh${ch.chIdx + 1}Value`);
  ...
  /**
   * Stop chart data capture — Continuous(FC 0x64) 또는 Trigger(FC 0x65) 모드
   * 분기
   */
  ...
  async startTriggerCapture() {
    ...
    const configFrame = this.modbus.buildTriggerConfigure(
        slaveId, period, chSel4, sourceSelect, edge, position, level,
        numOfData);
    ...
    for (const {chIdx, chNum} of configuredChannels) {
      ...
      while (startAddr < numOfData) {
        const reqFrame =
            this.modbus.buildTriggerDataRequest(slaveId, chIdx, startAddr);
        const resp = await this.sendAndReceiveFC65(reqFrame, 0x03, 500);

        if (!resp) {
          this.showToast(
              `CH${chIdx + 1} 데이터 수신 실패 (addr=${startAddr})`, 'warning');
          break;
        }
   ...
   * triggerRunning = false 로 설정하면 폴링 루프가 스스로 종료되며 Stop
   * 프레임을 전송한다.
   */
  ...
  initMiniCharts() {
    const init = (canvasId, chartKey, channels) => {
      ...
      // offsetWidth/offsetHeight: display:flex 설정 직후 호출 시 layout
      // reflow로 올바른 값 반환 canvas 버퍼를 devicePixelRatio 배율로 설정해야
      // HiDPI 화면에서 선이 선명하게 렌더링됨
      const dpr = window.devicePixelRatio || 1;
      ...
      // 숨겨진 탭의 canvas는 offsetWidth/Height = 0 → 잘못된 크기로 초기화되어
      // 블러 발생 이 경우 건너뛰고, 해당 탭이 표시될 때(startMiniChart 내 lazy
      // init) 재시도
      if (!w || !h) return;
      canvas.width = w * dpr;
      canvas.height = h * dpr;
    ...
    const saved = (() => {
      try {
        return JSON.parse(localStorage.getItem('ovInverterSettings') || '{}');
      } catch {
        return {};
      }
    })();
  ...
  async startMiniChart(type) {
    ...
    // stopMiniChart가 _fc64Busy를 해제했더라도 configure 프레임 전송 전까지
    // 버스 점유 유지
    this._fc64Busy = true;
    ...
    if (!chart) {
      ...
      this.showToast(
          '차트 초기화 실패: HW Overview 탭을 다시 열어주세요', 'error');
      return;
    }
    chart.clear();
    ...
    if (type === 'offsetHall') {
      ...
      if (polarCanvas) {
        const dpr = window.devicePixelRatio || 1;
        const pw = polarCanvas.offsetWidth ||
            polarCanvas.parentElement?.offsetWidth || 260;
        const ph = polarCanvas.offsetHeight ||
            polarCanvas.parentElement?.offsetHeight || 260;
    ...
    this.miniChartRunning[type] =
        true;  // 이후 _isFc64Active는 miniChartRunning으로 유지됨
    this._fc64Busy = false;
    if (type === 'current') {
      this._currentRmsData =
          [{sumSq: 0, count: 0}, {sumSq: 0, count: 0}, {sumSq: 0, count: 0}];
    ...
    while (this.miniChartRunning[type]) {
      ...
      for (let s = 0; s < samplesPerCh; s++) {
        for (let ci = 0; ci < numCh; ci++) {
          ...
          if (s === samplesPerCh - 1) {
            ...
            const minEl =
                minIds[ci] ? document.getElementById(minIds[ci]) : null;
            const maxEl =
                maxIds[ci] ? document.getElementById(maxIds[ci]) : null;
            if (minEl && ch.min !== undefined)
              minEl.textContent = ch.min.toFixed(3);
            if (maxEl && ch.max !== undefined)
              maxEl.textContent = ch.max.toFixed(3);
            // RMS 표시 업데이트
            if (rmsIds) {
      ...
      if (type === 'offsetHall' && this.offsetPolarChart && samplesPerCh > 0) {
        ...
        if (pol.lastAmp !== undefined) {
          ...
          if (pol.ampMin !== undefined)
            setTxt('offset-polar-amp-min', pol.ampMin.toFixed(3));
          if (pol.ampMax !== undefined)
            setTxt('offset-polar-amp-max', pol.ampMax.toFixed(3));
          if (pol.ampMin !== undefined && pol.ampMax !== undefined) {
            const sum = pol.ampMax + pol.ampMin;
            const ecc = sum > 0 ?
                ((pol.ampMax - pol.ampMin) / sum * 100).toFixed(1) :
                '-';
            setTxt('offset-polar-ecc', ecc);
            const eccEl = document.getElementById('offset-polar-ecc');
            if (eccEl) {
              const v = parseFloat(ecc);
              eccEl.style.color = v < 5 ? '#28a745' :
                  v < 15                ? '#fd7e14' :
                                          '#dc3545';
            }
          }
  ...
  async stopMiniChart(type) {
    ...
    if (type === 'offsetHall') {
      ['offset-polar-amp', 'offset-polar-amp-min', 'offset-polar-amp-max',
       'offset-polar-ecc', 'offset-polar-angle']
          .forEach(id => {
            const el = document.getElementById(id);
            if (el) el.textContent = '-';
          });
  ...
  _getMiniChartSlaveId() {
    const dev = this.devices.find(d => d.id === this.currentSetupDeviceId) ||
        this.devices[0];
    ...
    if (type === 'offsetHall') {
      ...
      ['offsetCard-hall', 'offsetCard-polar'].forEach(id => {
        document.getElementById(id)?.classList.toggle(
            'ov-card-running', running);
      });
      return;
    }
    const cardId = type === 'hall' ? 'ovCard-ps-hall-sensor' :
                                     'ovCard-ps-inverter-current';
    document.getElementById(cardId)?.classList.toggle(
        'ov-card-running', running);
  }

  // CANopen 파라미터 전송만 담당 — 차트 제어는 카드 클릭(toggleMiniChart)이
  // 담당
  async startOvInverter() {
    ...
    const agingCurrent = parseInt(
        document.getElementById('ovInverterCurrent')?.value ?? '100', 10);
    const agingSpeed =
        parseInt(document.getElementById('ovInverterSpeed')?.value ?? '10', 10);
  ...
  async runUsbHidTest() {
    if (!navigator.hid) {
      this.showToast(
          'WebHID를 지원하지 않는 브라우저입니다 (Chrome/Edge 필요)', 'error');
      return;
    }
    ...
    try {
      ...
      if (!this.hidDevice || !this.hidDevice.opened) {
        ...
        this.showToast(
            `HID 장치 선택됨: ${this.hidDevice.productName}`, 'info');
      }
      ...
      console.log(
          '[USB HID] device:', this.hidDevice.productName,
          '| collections:', collections.length, '| outputReports:',
          collections
              .map(
                  c => c.outputReports?.map(
                      r => `id=0x${r.reportId.toString(16)} items=${
                          r.items?.length}`))
              .flat());

      // Report ID 자동 감지: outputReports에서 첫 번째 ID 사용, 없으면 0x00
      // (no-report-ID 장치)
      const outputReport = collections[0]?.outputReports?.[0];
      const reportId = outputReport?.reportId ?? 0x00;

      // 패킷 크기: reportId=0이면 no-report-ID → data=65bytes, 아니면
      // data=64bytes (reportId 자동 prepend)
      const packetSize = reportId === 0 ? 65 : 64;
      const data = new Uint8Array(packetSize);
      if (reportId === 0) {
        // no-report-ID 장치: data[0]=0x00(dummy), data[1..] = frame (slaveId
        // 제외)
      ...
      console.log(`[USB HID] sendReport(reportId=0x${
          reportId.toString(16)}, dataLen=${packetSize})`);
      this.showToast(
          `sendReport(id=0x${reportId.toString(16)}, ${packetSize}B) 전송 중…`,
          'info');
      ...
      // USB HID 응답은 Report ID가 WebHID에서 제거되어 event.data가 FC부터
      // 시작함 parseCANopenResponse는 [slaveId][FC][MEI]... 구조를 기대하므로
      // slaveId 앞에 삽입
      ...
      const parsed = this.modbus.parseCANopenResponse(
          fullResponse.slice(0, frameLen), {skipCRC: true});
      this.addMonitorEntry('received', fullResponse.slice(0, frameLen));

      const MOTOR_ID_MAP = {0x1000: 'Sirocco Motor', 0x2000: 'Axial Motor'};
      const hexVal =
          `0x${parsed.value.toString(16).toUpperCase().padStart(4, '0')}`;
  ...
  async runOvMotorId() {
    const device = this._getManufactureDevice();
    if (!device) {
      this.showToast('디바이스가 선택되지 않았습니다', 'error');
      return;
    }
    ...
    const hexVal = result.value != null ?
  ...
  getAlarmCodeName(code) {
    ...
    const hex =
        '0x' + (code & 0xFF).toString(16).toUpperCase().padStart(2, '0');
  ...
  async _offsetAlarmPollingLoop() {
    while (this.offsetAlarmPollingRunning) {
      ...
      if (this.autoPollingTimer || this._isFc64Active ||
        ...
        // 이 루프가 버스 소유자 — 직접 전송
        const frame =
            this.modbus.buildCANopenUpload(slaveId, 0x603F, 0x00, 0, 2);
  ...
  async _offsetStatus2617PollingLoop() {
    while (this.offsetStatus2617PollingRunning) {
      ...
      let result;
      // offsetAlarmPollingRunning도 버스 소유자 조건에 포함 — alarm 루프가 bus
      // owner일 때 큐 경유 alarm 루프의 _drainCommandQueue 호출이 이 루프의
      // 요청을 처리함
      if (this.autoPollingTimer || this._isFc64Active ||
          this.ovPollingRunning || this.offsetAlarmPollingRunning) {
        // 다른 루프가 버스 점유 중 — commandQueue 경유
        ...
        // 이 루프가 버스 소유자 — 직접 전송
        const frame =
            this.modbus.buildCANopenUpload(slaveId, 0x2617, 0x00, 0, 8);
        result = await this.sendCANopenAndWaitResponse(frame, slaveId);
        if (this.commandQueue.length > 0) await this._drainCommandQueue();
      }
      ...
      const el = document.getElementById('offsetStatus2617');
      if (el) {
        if (result && !result.error && result.rawBytes &&
            result.rawBytes.length >= 5) {
          el.textContent =
              this._seg7BytesToString(result.rawBytes).trimEnd() || '-';
        } else {
          el.textContent = '-';
        }
      }
  ...
  async runOvOsVersion() {
    ...
    const device = this._getManufactureDevice();
    if (!device) {
      this._setOvBadge('mcu-os-version', 'fail');
      return;
    }
    const slaveId = device.slaveId;

    // ── 버그 방지: runHardwareTest('hw-canopen-27f0')를 경유하지 않고 직접
    // 읽기 ── runHardwareTest 내부의 readCANopenObject 는 ovPollingRunning=true
    // 이면 큐에 등록하는데, OV 루프가 버스 소유자일 때(autoPollingTimer=false)
    // 아무도 큐를 소진하지 않아 데드락 발생. → autoPollingTimer/FC64 활성 시
    // 직접 큐 push, 아니면 직접 전송.
    const entries = [
    ...
    for (const e of entries) {
      ...
      const frame =
          this.modbus.buildCANopenUpload(slaveId, e.index, 0x00, 0, 16);
      let result;
      if (this.autoPollingTimer || this._isFc64Active) {
  ...
  renderDeviceSetupList() {
    ...
    this.devices.forEach((device, index) => {
      ...
      item.innerHTML = `
                <div style="flex: 1;">
                    <div class="device-setup-name" style="font-weight: 500; color: inherit; margin-bottom: 2px; pointer-events: none;">${
          device.name}</div>
  ...
  selectDeviceInSetup(deviceId) {
    ...
    listItems.forEach(item => {
      if (item.dataset.deviceId ==
          deviceId) {  // Use == for type coercion (dataset is string, deviceId
                       // is number)
        item.classList.add('selected');
  ...
  showConfigMenu(event, configType, deviceId) {
    ...
    menu.innerHTML = `
            ${
        canRead ? `
            <div class="config-menu-item" onclick="window.dashboard.readConfigParam('${
                      configType}', '${deviceId}')">
                읽기
            </div>
            ...
            ` :
                  ''}
            <div class="config-menu-item" onclick="window.dashboard.factoryResetConfig('${
        configType}', '${deviceId}')">
                Factory Reset
            </div>
        `;
  ...
  getConfigParamMap(deviceId) {
    ...
    return {
      motorType: {
        ...
        reader: async () => {
          const result =
              await this.readCANopenObject(device.slaveId, 0x2000, 0x00);
          return (result && !result.error && result.value != null) ?
              result.value :
              null;
        },
        apply: (raw) => {
  ...
  async readConfigParam(configType, deviceId) {
    ...
    const statusEl =
        document.getElementById(`${configType}_${deviceId}_status`);
    const inputEl = document.getElementById(`${configType}_${deviceId}`);

    if (statusEl) {
      statusEl.textContent = '↻';
      statusEl.title = 'Reading...';
    }
  ...
  factoryResetConfig(configType, deviceId) {
    console.log(
        `Factory reset requested for ${configType} on device ${deviceId}`);
    ...
    switch (category) {
      case 'motorInfo':
        return `<div style="margin-top: 0;">
                    ${
            row('motorType', '모터 타입',
                '연결된 모터의 타입 ID (FC 0x2B, 0x2000)',
                `
                        <select id="motorType_${id}" style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('motorType', ${
                    id})"
                            onclick="event.stopPropagation()">
                            <option value="" ${
                    device.motorType == null ?
                        'selected' :
                        ''} disabled>-- 선택 --</option>
                            <option value="4096" ${
                    device.motorType === 4096 ?
                        'selected' :
                        ''}>Sirocco Motor (0x1000)</option>
                            <option value="8192" ${
                    device.motorType === 8192 ?
                        'selected' :
                        ''}>Axial Motor (0x2000)</option>
                        </select>`)}
                </div>`;
            ...
            row('operatingMode', '동작 모드', '모터 제어 방식 설정 (0xD106)',
                `
                        <select id="operatingMode_${id}" style="${iStyle}"
                            onchange="window.dashboard.applyOperatingMode(${
                    id})"
                            onclick="event.stopPropagation()">
                            <option value="0" ${
                    device.operationMode === 0 ? 'selected' :
                                                 ''}>Speed Control</option>
                            <option value="2" ${
                    device.operationMode === 2 ? 'selected' :
                                                 ''}>Open-loop</option>
                        </select>`)}
                    ${
            row('setValueSource', 'Setpoint 입력 소스',
                'Setpoint를 수신하는 입력 수단 (0xD101)',
                `
                        <select id="setValueSource_${id}" style="${iStyle}"
                            onchange="window.dashboard.applySetValueSource(${
                    id})"
                            onclick="event.stopPropagation()">
                            <option value="0" ${
                    (device.setValueSource ?? 1) === 0 ? 'selected' :
                                                         ''}>AIN1</option>
                            <option value="1" ${
                    (device.setValueSource ?? 1) === 1 ? 'selected' :
                                                         ''}>RS485</option>
                            <option value="2" ${
                    (device.setValueSource ?? 1) === 2 ? 'selected' :
                                                         ''}>AIN2</option>
                            <option value="3" ${
                    (device.setValueSource ?? 1) === 3 ? 'selected' :
                                                         ''}>PWM</option>
                        </select>`)}
                    ${
            ...
            row('maxSpeed', '최대 속도 (RPM)',
                ...
                `
                        <input type="number" id="maxSpeed_${id}"
                            ...
                            min="0" max="65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('maxSpeed', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('rampUp', '가속 시간',
                '정지에서 목표 속도까지 가속하는 시간 (0xD11F)',
                `
                        <input type="number" id="rampUp_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('rampUp', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('rampDown', '감속 시간',
                '목표 속도에서 정지까지 감속하는 시간 (0xD120)',
                `
                        <input type="number" id="rampDown_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('rampDown', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                </div>`;
      ...
      case 'protection':
        return `<div style="margin-top: 0;">
            ...
            row('maxCurrent', '최대 코일 전류 (A)',
                '전류 제한 활성화 시 최대 코일 전류(rms) (0xD13B)',
                `
                        <input type="number" id="maxCurrent_${id}"
                            value="${device.maxCurrent || 0}"
                            min="0" max="100" step="0.1"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('maxCurrent', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            ...
            row('fanAddress', 'Fan Address', 'Modbus Slave ID (0xD100)',
                `
                        <input type="number" id="fanAddress_${id}"
                            ...
                            min="1" max="247"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('fanAddress', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('baudrate', 'Baudrate',
                'RS-485 통신 속도 — 드라이브 재시작 후 적용 (0xD149)',
                `
                        <select id="baudrate_${id}" style="${iStyle}"
                            onchange="window.dashboard.applyBaudrate(${id})"
                            onclick="event.stopPropagation()">
                            <option value="0" ${
                    (device.baudrate ?? 4) === 0 ? 'selected' :
                                                   ''}>1200 bps</option>
                            <option value="1" ${
                    (device.baudrate ?? 4) === 1 ? 'selected' :
                                                   ''}>2400 bps</option>
                            <option value="2" ${
                    (device.baudrate ?? 4) === 2 ? 'selected' :
                                                   ''}>4800 bps</option>
                            <option value="3" ${
                    (device.baudrate ?? 4) === 3 ? 'selected' :
                                                   ''}>9600 bps</option>
                            <option value="4" ${
                    (device.baudrate ?? 4) === 4 ?
                        'selected' :
                        ''}>19200 bps (기본값)</option>
                            <option value="5" ${
                    (device.baudrate ?? 4) === 5 ? 'selected' :
                                                   ''}>38400 bps</option>
                            <option value="6" ${
                    (device.baudrate ?? 4) === 6 ? 'selected' :
                                                   ''}>57600 bps</option>
                            <option value="7" ${
                    (device.baudrate ?? 4) === 7 ? 'selected' :
                                                   ''}>115200 bps</option>
                        </select>`)}
                    ${
            row('parity', 'Parity',
                '시리얼 데이터 비트/패리티/스톱 비트 — 드라이브 재시작 후 적용 (0xD14A)',
                `
                    ...
                    (device.parity ?? 0) === 0 ?
                        'selected' :
                        ''}>Data8 / Even / Stop1 (기본값)</option>
                            <option value="1" ${
                    (device.parity ?? 0) === 1 ?
                        'selected' :
                        ''}>Data8 / Odd / Stop1</option>
                            <option value="2" ${
                    (device.parity ?? 0) === 2 ?
                        'selected' :
                        ''}>Data8 / None / Stop2</option>
                            <option value="3" ${
                    (device.parity ?? 0) === 3 ?
                        'selected' :
                        ''}>Data8 / None / Stop1</option>
                        </select>`)}
                    ${
            row('terminationResistor', '종단 저항',
                'RS-485 통신 종단 저항 활성화 설정 (0xD1FF)',
                `
                        <select id="terminationResistor_${id}" style="${
                    iStyle}"
                            onchange="window.dashboard.applyTerminationResistor(${
                    id})"
                            onclick="event.stopPropagation()">
                            <option value="0" ${
                    (device.terminationResistor ?? 0) === 0 ?
                        'selected' :
                        ''}>비활성화 (기본값)</option>
                            <option value="1" ${
                    (device.terminationResistor ?? 0) === 1 ?
                        'selected' :
                        ''}>활성화</option>
                        </select>`)}
                </div>`;
      ...
      case 'sensor':
        return `<div style="margin-top: 0;">
                    ${
            row('sensorActualSource', '센서 지령 소스',
                '지령으로 선택된 센서의 입력 채널 (0xD147)',
                `
                        <input type="number" id="sensorActualSource_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('sensorActualSource', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('minSensorValue', '센서 최솟값', '입력 센서의 최솟값 (0xD160)',
                `
                        <input type="number" id="minSensorValue_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('minSensorValue', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('maxSensorValue', '센서 최댓값', '입력 센서의 최댓값 (0xD162)',
                `
                        <input type="number" id="maxSensorValue_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('maxSensorValue', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('curvePoint1X', '커브 포인트 1 X',
                '아날로그/PWM 입력 신호 할당 — Point 1 X (0xD12A)',
                `
                        <input type="number" id="curvePoint1X_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('curvePoint1X', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('curvePoint1Y', '커브 포인트 1 Y',
                '아날로그/PWM 입력 신호 할당 — Point 1 Y (0xD12B)',
                `
                        <input type="number" id="curvePoint1Y_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('curvePoint1Y', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('curvePoint2X', '커브 포인트 2 X',
                '아날로그/PWM 입력 신호 할당 — Point 2 X (0xD12C)',
                `
                        <input type="number" id="curvePoint2X_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('curvePoint2X', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('curvePoint2Y', '커브 포인트 2 Y',
                '아날로그/PWM 입력 신호 할당 — Point 2 Y (0xD12D)',
                `
                        <input type="number" id="curvePoint2Y_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('curvePoint2Y', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                </div>`;
      ...
      case 'servoTuning':
        return `<div style="margin-top: 0;">
                    ${
            row('inertiaRatio', '관성비 (Inertia Ratio)',
                '부하 관성 / 모터 관성 비율 — 서보 응답성 튜닝 (FC 0x2B, 0x2100)',
                `
                        <input type="number" id="inertiaRatio_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('inertiaRatio', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('posPGain', '위치 P 게인 1',
                '위치 제어 루프 비례 게인 (FC 0x2B, 0x2101)',
                `
                        <input type="number" id="posPGain_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('posPGain', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('velPGain', '속도 P 게인 1',
                '속도 제어 루프 비례 게인 (FC 0x2B, 0x2102)',
                `
                        <input type="number" id="velPGain_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('velPGain', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('velTimeConst', '속도 루프 적분 시상수 1',
                '속도 루프 적분 시간 상수 (FC 0x2B, 0x2103)',
                `
                        <input type="number" id="velTimeConst_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('velTimeConst', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('torqueCmdFilter', '토크 지령 필터 시상수 1',
                '토크 지령 저역 통과 필터 시간 상수 (FC 0x2B, 0x2104)',
                `
                        <input type="number" id="torqueCmdFilter_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('torqueCmdFilter', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('posTorqueLimit', '양방향 토크 제한 (CW)',
                '외부 입력 기반 CW 방향 토크 상한값 (FC 0x2B, 0x2111)',
                `
                        <input type="number" id="posTorqueLimit_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('posTorqueLimit', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('negTorqueLimit', '음방향 토크 제한 (CCW)',
                '외부 입력 기반 CCW 방향 토크 상한값 (FC 0x2B, 0x2112)',
                `
                        <input type="number" id="negTorqueLimit_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('negTorqueLimit', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('driveCtrlInput1', '드라이브 제어 입력 1',
                '드라이브 제어 입력 포트 1 기능 할당 (FC 0x2B, 0x211F)',
                `
                        <input type="number" id="driveCtrlInput1_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('driveCtrlInput1', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                    ${
            row('driveCtrlInput2', '드라이브 제어 입력 2',
                '드라이브 제어 입력 포트 2 기능 할당 (FC 0x2B, 0x2120)',
                `
                        <input type="number" id="driveCtrlInput2_${id}"
                            ...
                            placeholder="0–65535"
                            style="${iStyle}"
                            onchange="window.dashboard.debouncedApply('driveCtrlInput2', ${
                    id})"
                            onclick="event.stopPropagation()">`)}
                </div>`;
      ...
      case 'system':
        return `<div style="margin-top: 0;">
                    ${
            actionRow(
                'softwareReset', '소프트웨어 리셋',
                '디바이스 소프트웨어를 재시작합니다 (0xD000)',
                `
  ...
  renderDeviceSetupParams(device) {
    ...
    const holdingParams = [
      ...
      },
      {
        ...
        address: 0xD001,
        writable: true,
        description: '지령 값 (RPM or %)'
      },
      {
        name: 'Fan Address',
        address: 0xD100,
        writable: true,
        description: 'Node ID (1-247)'
      },
      {
        name: 'Preferred Running Direction',
        address: 0xD102,
        writable: true,
        description: '0: CCW, 1: CW'
      },
      {
        name: 'Operating Mode',
        address: 0xD106,
        writable: true,
        description: '0: Speed Control, 2: Open-loop'
      },
      {
        name: 'Maximum Coil Current',
        address: 0xD13B,
        writable: true,
        description: 'Current limit (RMS)'
      }
    ];
    ...
    const inputParams = [
      ...
      {
        name: 'Bus Controller SW Name',
        address: 0xD002,
        writable: false,
        description: 'Main 부트버전'
      },
      {
        name: 'Bus Controller SW Version',
        address: 0xD003,
        writable: false,
        description: 'Main 펌웨어 버전'
      },
      {
        name: 'Commutation Controller SW Name',
        address: 0xD004,
        writable: false,
        description: 'Inverter 부트 버전'
      },
      {
        name: 'Commutation Controller SW Version',
        address: 0xD005,
        writable: false,
        description: 'Inverter 펌웨어 버전'
      },
      ...
      },
      {
        ...
        address: 0xD015,
        writable: false,
        description: 'IGBT Temperature'
      },
      {
        name: 'Electronics Temperature',
        address: 0xD017,
        writable: false,
        description: '제어부 Temperature'
      },
      {
        name: 'Actual Speed [RPM]',
        address: 0xD02D,
        writable: false,
        description: '절대 속도 [RPM]'
      },
    ...
    paramsContainer.innerHTML = `
        ...
        device.name}</div>
                    <div style="font-size: 12px; color: #6c757d;">Slave ID: ${
        device.slaveId === 0 ? 'Not Assigned' : device.slaveId}</div>
                </div>
            ...
            .map(
                param => `
                    ...
                    param.description}</div>
                                    <div style="font-size: 11px; color: #007bff; margin-top: 2px;">Address: 0x${
                    param.address.toString(16).toUpperCase()}</div>
                                </div>
                                <div style="display: flex; gap: 6px; align-items: center;">
                                    <input type="number" id="param_${
                    param.address}" placeholder="Value"
                                    ...
                                    <button class="btn btn-sm btn-primary" style="padding: 6px 12px; font-size: 11px;"
                                        onclick="window.dashboard.readParameter(${
                    device.slaveId}, ${param.address}, true)">Read</button>
                                    ${
                    param.writable ?
                        `
                                        <button class="btn btn-sm btn-success" style="padding: 6px 12px; font-size: 11px;"
                                            onclick="window.dashboard.writeParameter(${
                            device.slaveId}, ${
                            param.address}, document.getElementById('param_${
                            param.address}').value)">Write</button>
                                    ` :
                        ''}
                                </div>
                            </div>
            ...
            .map(
                param => `
                    ...
                    param.name}</div>
                                    <div style="font-size: 11px; color: #6c757d;">${
                    param.description}</div>
                                    <div style="font-size: 11px; color: #17a2b8; margin-top: 2px;">Address: 0x${
                    param.address.toString(16).toUpperCase()}</div>
                                </div>
                                <div style="display: flex; gap: 6px; align-items: center;">
                                    <input type="number" id="param_input_${
                    param.address}" placeholder="Value" readonly
                                    ...
                                    <button class="btn btn-sm btn-info" style="padding: 6px 12px; font-size: 11px;"
                                        onclick="window.dashboard.readParameter(${
                    device.slaveId}, ${param.address}, false)">Read</button>
                                </div>
                            </div>
                        `)
            .join('')}
  ...
  async readParameter(slaveId, address, isHolding) {
    try {
      ...
      if (value !== null && value !== undefined) {
        const inputId =
            isHolding ? `param_${address}` : `param_input_${address}`;
  ...
  debouncedApply(paramType, deviceId) {
    ...
    const statusElement =
        document.getElementById(`${paramType}_${deviceId}_status`);
      ...
      try {
        switch (paramType) {
          ...
          case 'tempDeratingStart':
            await this.applyGenericHolding(
                deviceId, 'tempDeratingStart', 0xD137, 'tempDeratingStart',
                {max: 200});
            success = true;
            break;
          // ── 센서 입력 ────────────────────────────────────────
          case 'sensorActualSource':
            await this.applyGenericHolding(
                deviceId, 'sensorActualSource', 0xD147, 'sensorActualSource');
            success = true;
            break;
          case 'minSensorValue':
            await this.applyGenericHolding(
                deviceId, 'minSensorValue', 0xD160, 'minSensorValue');
            success = true;
            break;
          case 'maxSensorValue':
            await this.applyGenericHolding(
                deviceId, 'maxSensorValue', 0xD162, 'maxSensorValue');
            success = true;
            break;
          case 'curvePoint1X':
            await this.applyGenericHolding(
                deviceId, 'curvePoint1X', 0xD12A, 'curvePoint1X');
            success = true;
            break;
          case 'curvePoint1Y':
            await this.applyGenericHolding(
                deviceId, 'curvePoint1Y', 0xD12B, 'curvePoint1Y');
            success = true;
            break;
          case 'curvePoint2X':
            await this.applyGenericHolding(
                deviceId, 'curvePoint2X', 0xD12C, 'curvePoint2X');
            success = true;
            break;
          case 'curvePoint2Y':
            await this.applyGenericHolding(
                deviceId, 'curvePoint2Y', 0xD12D, 'curvePoint2Y');
            success = true;
            break;
          // ── 서보 튜닝 (LSM) ──────────────────────────────────
          case 'lsmNodeId':
            await this.applyGenericLSM(
                deviceId, 'lsmNodeId', 0x2003, 'lsmNodeId');
            success = true;
            break;
          case 'inertiaRatio':
            await this.applyGenericLSM(
                deviceId, 'inertiaRatio', 0x2100, 'inertiaRatio');
            success = true;
            break;
          case 'posPGain':
            await this.applyGenericLSM(
                deviceId, 'posPGain', 0x2101, 'posPGain');
            success = true;
            break;
          case 'velPGain':
            await this.applyGenericLSM(
                deviceId, 'velPGain', 0x2102, 'velPGain');
            success = true;
            break;
          case 'velTimeConst':
            await this.applyGenericLSM(
                deviceId, 'velTimeConst', 0x2103, 'velTimeConst');
            success = true;
            break;
          case 'torqueCmdFilter':
            await this.applyGenericLSM(
                deviceId, 'torqueCmdFilter', 0x2104, 'torqueCmdFilter');
            success = true;
            break;
          case 'posTorqueLimit':
            await this.applyGenericLSM(
                deviceId, 'posTorqueLimit', 0x2111, 'posTorqueLimit');
            success = true;
            break;
          case 'negTorqueLimit':
            await this.applyGenericLSM(
                deviceId, 'negTorqueLimit', 0x2112, 'negTorqueLimit');
            success = true;
            break;
          case 'driveCtrlInput1':
            await this.applyGenericLSM(
                deviceId, 'driveCtrlInput1', 0x211F, 'driveCtrlInput1');
            success = true;
            break;
  ...
  async applyFanAddress(deviceId) {
    ...
    const statusElement =
        document.getElementById(`fanAddress_${deviceId}_status`);
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD100, newAddress);
    if (success) {
      device.slaveId = newAddress;
      // Keep selectedParamDeviceId in sync: if it was pointing at the old
      // address, update it so Read All continues to target the correct device.
      ...
      const dashboardCard =
          document.querySelector(`.device-card[data-device-id="${
              deviceId}"], .device-list-item[data-device-id="${deviceId}"]`);
      if (dashboardCard) {
  ...
  async applyOperatingMode(deviceId) {
    ...
    const statusElement =
        document.getElementById(`operatingMode_${deviceId}_status`);
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD106, newMode);
    if (success) {
      device.operationMode = newMode;
      this.saveDevices();
      this.renderDeviceSetupList();
  ...
  async applyRunningDirection(deviceId) {
    ...
    const selectElement =
        document.getElementById(`runningDirection_${deviceId}`);
    const statusElement =
        document.getElementById(`runningDirection_${deviceId}_status`);
    if (!selectElement) return;
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD102, newDirection);
    if (success) {
  ...
  async applySetpoint(deviceId) {
    ...
    const statusElement =
        document.getElementById(`setpoint_${deviceId}_status`);
    if (!inputElement) return;
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD001, newSetpoint);
    if (success) {
  ...
  async applyMaxCurrent(deviceId) {
    ...
    const statusElement =
        document.getElementById(`maxCurrent_${deviceId}_status`);
    if (!inputElement) return;
    ...
    const registerValue =
        Math.round(newCurrent * 10);  // Assuming 0.1A resolution

    // Write to 0xD13B (Maximum coil current)
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD13B, registerValue);
    if (success) {
  ...
  async applySetValueSource(deviceId) {
    ...
    if (status) {
      status.textContent = '↻';
      status.title = 'Applying...';
    }

    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD101, value);
    if (success) {
      device.setValueSource = value;
      this.saveDevices();
      if (status) {
        status.textContent = '⭕';
        status.title = 'Applied successfully';
      }
    } else {
      if (status) {
        status.textContent = '❌';
        status.title = 'Failed to apply';
      }
    }
  ...
  async applyMaxSpeed(deviceId) {
    ...
    if (isNaN(value) || value < 0 || value > 65535) {
      if (status) {
        status.textContent = '❌';
        status.title = 'Invalid value (0–65535)';
      }
      return;
    }
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD119, value);
    if (success) {
  ...
  async applyRampUp(deviceId) {
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD11F, value);
    if (success) {
      device.rampUp = value;
      this.saveDevices();
      if (status) {
  ...
  async applyRampDown(deviceId) {
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD120, value);
    if (success) {
  ...
  async applyTempDerating(deviceId) {
    ...
    if (isNaN(value) || value < 0 || value > 200) {
      if (status) {
        status.textContent = '❌';
        status.title = 'Invalid value (0–200°C)';
      }
      return;
    }
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD138, value);
    if (success) {
  ...
  async applyMotorType(deviceId) {
    ...
    const result =
        await this.writeCANopenObject(device.slaveId, 0x2000, 0x00, value);
    if (result && !result.error) {
  ...
  async applyBaudrate(deviceId) {
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD149, value);
    if (success) {
  ...
  async applyParity(deviceId) {
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD14A, value);
    if (success) {
      device.parity = value;
      this.saveDevices();
      if (status) {
   ...
   * @param {object} opts      - { min, max, scale } (scale multiplies the value
   *     before writing)
   */
  async applyGenericHolding(
      deviceId, paramKey, address, deviceProp, opts = {}) {
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, address, regVal);
    if (success) {
  ...
  async applyGenericLSM(deviceId, paramKey, lsmIndex, deviceProp) {
    ...
    const result =
        await this.writeCANopenObject(device.slaveId, lsmIndex, 0x00, val);
    if (result && !result.error) {
      if (deviceProp) device[deviceProp] = val;
      this.saveDevices();
      if (status) {
  ...
  async applyTerminationResistor(deviceId) {
    ...
    const status =
        document.getElementById(`terminationResistor_${deviceId}_status`);
    if (!sel) return;
    const value = parseInt(sel.value);
    if (status) {
      status.textContent = '↻';
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD1FF, value);
    if (success) {
  ...
  async resetDevice(deviceId, resetType) {
    ...
    switch (resetType) {
      case 'software':
        ...
        confirmMessage =
            'Are you sure you want to perform a software reset? The device will restart.';
        break;
      case 'error':
      ...
      case 'eeprom':
        ...
        confirmMessage =
            'Are you sure you want to load settings from EEPROM to RAM?';
        break;
      default:
        return;
    }
    ...
    const success =
        await this.writeHoldingRegister(device.slaveId, 0xD000, resetValue);
    if (success) {
      this.showToast(
          `${
              resetType.charAt(0).toUpperCase() +
              resetType.slice(1)} 리셋이 완료되었습니다`,
          'success');
  ...
  _setHwTestResult(testId, status, message) {
    const item =
        document.querySelector(`.hw-test-item[data-test-id="${testId}"]`);
    if (!item) return;
  ...
  _updateHwTestCounter() {
    ...
    items.forEach(item => {
      const badge = item.querySelector('.hw-test-badge');
      if (!badge) return;
      if (badge.textContent === 'Pass')
        pass++;
      else if (badge.textContent === 'Fail')
        fail++;
      else
        pending++;
    });
    ...
    if (el('hwTestProgress'))
      el('hwTestProgress').textContent =
  ...
  snOnTabOpen() {
    ...
    if (statusEl) {
      statusEl.textContent = '미확인 — 패스워드를 입력 후 확인 버튼을 누르세요';
      statusEl.style.color = '#b0b8c1';
    }
    const pwInput = document.getElementById('snPassword');
    if (pwInput) {
      pwInput.value = '';
      pwInput.style.borderColor = '#e8eaed';
    }
  ...
  async snConfirmPassword() {
    ...
    if (!device) {
      if (statusEl) {
        statusEl.textContent = '✘ 디바이스가 선택되지 않았습니다';
    ...
    if (isNaN(pwValue)) {
      if (statusEl) {
        statusEl.textContent = '✘ 숫자를 입력해주세요';
        statusEl.style.color = '#f04452';
      }
      return;
    }
    ...
    if (statusEl) {
      statusEl.textContent = '디바이스에 패스워드 전송 중...';
      statusEl.style.color = '#8b95a1';
    }

    try {
      // FC 0x2B CANopen Download: index=0x4009, subIndex=0x00, value=입력값
      const result = await this.writeCANopenObject(
          device.slaveId, 0x4009, 0x00, [pwValue]);
      if (!result || result.error)
        throw new Error(result?.error || '응답 없음');

      this._snUnlocked = true;
      if (statusEl) {
        statusEl.textContent = '✔ 패스워드가 입력되었습니다';
  ...
  _snUpdateWriteBtn() {
    const serial =
        (document.getElementById('snSerialInput')?.value || '').toUpperCase();
  ...
  snOnSerialInput(el) {
    ...
    if (raw.length === 0) {
      ...
      if (msgEl) {
        msgEl.textContent =
            '형식: 제품(S/M/D/R) + 연도코드(2자) + 월코드(A-L) + 일련번호(5자리) = 9자';
        msgEl.style.color = '#b0b8c1';
      }
    } else if (ModbusDashboard.SN_REGEX.test(raw)) {
      inputEl.style.borderColor = '#00c471';
      if (msgEl) {
        msgEl.textContent = '✔ 유효한 형식입니다';
        msgEl.style.color = '#00c471';
      }
    } else {
  ...
  _snValidationHint(s) {
    ...
    if (!/^[SMDR][A-Z]/.test(s))
      return '두 번째 글자는 연도 십년 코드 (A=2000s, B=2010s, …)';
    if (!/^[SMDR][A-Z]\d/.test(s))
      return '세 번째 글자는 연도 끝자리 숫자 (0-9)';
    if (!/^[SMDR][A-Z]\d[A-L]/.test(s))
      return '네 번째 글자는 월 코드 (A=1월 … L=12월)';
    if (!/^[SMDR][A-Z]\d[A-L]\d{5}$/.test(s))
      return '다섯 번째부터 끝까지 5자리 숫자여야 합니다';
    return '형식 오류';
  }
  ...
  _snUpdateDeviceLabel() {
    ...
    el.textContent = device ?
        `${device.name || ('Fan #' + device.id)} (Slave ${device.slaveId})` :
        '선택된 디바이스 없음';
  }
  ...
  snGenerate() {
    ...
    if (!product || isNaN(year) || year < 2000 || isNaN(month) || month < 1 ||
        month > 12) {
    ...
    if (decodedEl)
      decodedEl.textContent = `${year}년 ${month}월 / 일련번호 ${seq}번`;
  ...
  async snWriteToDevice() {
    ...
    const serial =
        (document.getElementById('snSerialInput')?.value || '').toUpperCase();
    if (!ModbusDashboard.SN_REGEX.test(serial)) {
      alert('유효한 시리얼 넘버를 입력해주세요.');
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < 8; i++) {
        words.push(
            (padded.charCodeAt(i * 2) << 8) | padded.charCodeAt(i * 2 + 1));
      }
      // FC 0x2B CANopen Download: index=0x2424, subIndex=0x00
      const result =
          await this.writeCANopenObject(device.slaveId, 0x2424, 0x00, words);
      if (!result || result.error)
        throw new Error(result?.error || '응답 없음');
      if (statusEl) {
  ...
  async snReadFromDevice() {
    ...
    if (outputEl) {
      outputEl.textContent = '읽는 중...';
      outputEl.style.color = '#b0b8c1';
    }
    if (statusEl) {
      statusEl.textContent = '';
      statusEl.style.color = '#b0b8c1';
  ...
  _snRenderDecode(serial) {
    ...
    try {
      const pMap =
          {S: '인코더 (S)', M: '모터 (M)', D: '드라이브 (D)', R: '로봇 (R)'};
      ...
      list.innerHTML = `
                <li style="display:flex;justify-content:space-between;align-items:center;padding:12px 20px;border-bottom:1px solid #f5f6f8;font-size:13px;">
                    ...
                    <span style="font-weight:600;color:#191f28;">${
          pName}</span>
                </li>
                <li style="display:flex;justify-content:space-between;align-items:center;padding:12px 20px;border-bottom:1px solid #f5f6f8;font-size:13px;">
                    <span style="color:#8b95a1;font-weight:500;">제작년월</span>
                    <span style="font-weight:600;color:#191f28;">${
          fullYear}년 ${month}월</span>
                </li>
      ...
      list.innerHTML =
          `<li style="padding:12px 20px;font-size:13px;color:#f04452;">해독 실패 — 형식이 올바르지 않습니다</li>`;
    }
  }
    ...
    try {
      switch (testId) {
        case 'hw-inv-os': {
          ...
          const expected =
              document.getElementById('hwInvOsExpected')?.value?.trim();
          const el = document.getElementById('hwInvOsActual');
          if (el)
            el.textContent =
                '0x' + raw.toString(16).toUpperCase().padStart(4, '0');
          if (!expected) {
            this._setHwTestResult(testId, 'fail', '기준 버전을 입력해주세요');
            return;
          }
          const expVal = parseInt(expected, 16);
          const ok = raw === expVal;
          this._setHwTestResult(
              testId, ok ? 'pass' : 'fail',
              ok ? `Pass — 읽은 값 0x${raw.toString(16).toUpperCase()} 일치` :
                   `Fail — 읽은 값 0x${raw.toString(16).toUpperCase()}, 기준 ${
                       expected}`);
          break;
        }
        case 'hw-main-os': {
          ...
          const expected =
              document.getElementById('hwMainOsExpected')?.value?.trim();
        ...
        case 'hw-motor-id': {
          ...
          const expected =
              document.getElementById('hwMotorIdExpected')?.value?.trim();
        ...
        case 'hw-eeprom': {
          ...
          await this.writeRegister(slaveId, 0x2002, 110);
          await this.writeRegister(
              slaveId, 0x1010, 0x65766173);  // Store Parameters
          await new Promise(r => setTimeout(r, 500));
          const readBack = await this.readRegisterWithTimeout(slaveId, 0x2002);
          ...
          this._setHwTestResult(
              testId, ok ? 'pass' : 'fail',
              ok ? 'Pass — Write 110, Read 110 일치' :
                   `Fail — Write 110, Read ${readBack}`);
          break;
        }
        case 'hw-igbt-temp': {
        ...
        case 'hw-hall': {
          ...
          this._setHwTestResult(
              testId, ok ? 'pass' : 'fail',
              ok ? `Pass — H1:${h1}, H2:${h2}, H3:${h3} (1100~2000)` :
                   `Fail — H1:${h1}, H2:${h2}, H3:${h3}`);
          break;
        }
        case 'hw-current': {
          ...
          const ratedInput = parseFloat(
              document.getElementById('hwCurrentRated')?.value || '0');
          if (!ratedInput) {
            this._setHwTestResult(testId, 'fail', '정격 전류를 입력해주세요');
            return;
          }
          ...
          this._setHwTestResult(
              testId, ok ? 'pass' : 'fail',
              ok ? `Pass — IU:${iu}, IV:${iv}, IW:${iw}` :
                   `Fail — IU:${iu}, IV:${iv}, IW:${iw} (목표범위 ${
        ...
        case 'hw-factory-reset': {
          ...
          await this.writeRegister(slaveId, 0x1010, 0x65766173);
          this._setHwTestResult(
              testId, 'pass', 'Pass — Factory Reset 및 파라미터 저장 완료');
          break;
        }
        case 'hw-canopen-27f0': {
          const indices = [0x27F0, 0x27F1, 0x27F2, 0x27F3];
          const elIds = [
            'hwCanopen27F0Value', 'hwCanopen27F1Value', 'hwCanopen27F2Value',
            'hwCanopen27F3Value'
          ];
          const rawIds = [
            'hwCanopen27F0Raw', 'hwCanopen27F1Raw', 'hwCanopen27F2Raw',
            'hwCanopen27F3Raw'
          ];
          const results = [];
          let allPass = true;
          for (let i = 0; i < indices.length; i++) {
            ...
            const r =
                await this.readCANopenObject(slaveId, indices[i], 0x00, 16);
            if (!r || r.error) {
              ...
              const ascii = r.rawBytes.filter(b => b !== 0x00)
                                .map(
                                    b => (b >= 0x20 && b < 0x7F) ?
              ...
              results.push(
                  `0x${indices[i].toString(16).toUpperCase()}="${ascii}"`);
            }
          }
          if (allPass) {
            this._setHwTestResult(
                testId, 'pass', `Pass — ${results.join(', ')}`);
          } else {
            this._setHwTestResult(
                testId, 'fail', `Fail — ${results.join(', ')}`);
          }
          break;
        }
        case 'hw-canopen-motor-id': {
          ...
          if (elRaw) elRaw.textContent = rawHex;
          this._setHwTestResult(
              testId, 'pass', `Pass — ${motorName} (${hexVal})`);
          break;
        }
        case 'hw-canopen-2000': {
          ...
          this._setHwTestResult(
              testId, 'pass', `Pass — 값 ${hexVal} (raw: ${rawHex})`);
          break;
        }
        case 'hw-write-cmd-test': {
          // Power Stack-03: Inverter 출력 및 전류 센싱 (FC 0x2B CANopen SDO
          // Download)
          const agingCurrent = parseInt(
              document.getElementById('hwWriteCmdCurrent')?.value || '100', 10);
          const agingSpeed = parseInt(
              document.getElementById('hwWriteCmdSpeed')?.value || '10', 10);
          if (isNaN(agingCurrent) || agingCurrent < 0 || agingCurrent > 100) {
            this._setHwTestResult(
                testId, 'fail', 'Aging Current 값을 확인해주세요 (0~100 %)');
            return;
          }
          if (isNaN(agingSpeed) || agingSpeed < 0) {
            this._setHwTestResult(
                testId, 'fail', 'Aging Speed 값을 확인해주세요 (0 Hz 이상)');
            return;
          }
          await this.writeCANopenObject(slaveId, 0x4004, 0x00, agingCurrent);
          await this.writeCANopenObject(slaveId, 0x4005, 0x00, agingSpeed);
          ...
          this._setHwTestResult(
              testId, 'pass',
              `Pass — FC 0x2B: 0x4004=${agingCurrent}%, 0x4005=${
                  agingSpeed}Hz, 0x2701=1, 0x2700=0x1000 전송 완료`);
          break;
        }

        case 'hw-ps-04': {
          // Power Stack-04: IPM / Board Temperature 읽기 (FC 0x2B CANopen SDO
          // Upload)
          const toInt16 = v => {
            const n = v & 0xFFFF;
            return n >= 0x8000 ? n - 0x10000 : n;
          ...
          this._setHwTestResult(
              testId, 'pass',
              `Pass — IPM Temp=${fmtTemp(t260B)}, Board Temp=${
                  fmtTemp(t260C)}`);
          break;
        }
  ...
  async stopPowerStack03() {
    ...
    if (!device) {
      this._setHwTestResult(
          'hw-write-cmd-test', 'fail', '디바이스가 선택되지 않았습니다');
      return;
    }
    const slaveId = device.slaveId;
    try {
      ...
      await this.writeCANopenObject(slaveId, 0x2700, 0x00, 0x1000);
      this._setHwTestResult(
          'hw-write-cmd-test', 'pending',
          'Stop 완료 — 0x4004=0, 0x4005=0, 0x2701=2, 0x2700=0x1000');
    } catch (err) {
      this._setHwTestResult(
          'hw-write-cmd-test', 'fail', `Stop 오류: ${err.message || err}`);
    }
  }

  async runAllHardwareTests() {
    const testIds = [
      'hw-canopen-27f0', 'hw-canopen-motor-id', 'hw-eeprom', 'hw-current',
      'hw-igbt-temp', 'hw-hall', 'hw-factory-reset', 'hw-canopen-2000'
    ];
  ...
  resetAllHardwareTests() {
    document.querySelectorAll('.hw-test-item').forEach(item => {
      ...
      this._setHwTestResult(
          testId, 'pending', '테스트를 실행하면 결과가 표시됩니다');
    });
    // Clear displayed values
    ['hwInvOsActual', 'hwMainOsActual', 'hwMotorIdActual', 'hwEepromActual',
     'hwIgbtTempActual', 'hwCurrentIU', 'hwCurrentIV', 'hwCurrentIW', 'hwHall1',
     'hwHall2', 'hwHall3']
        .forEach(id => {
          const el = document.getElementById(id);
          if (el) el.textContent = '-';
  ...
  _resetOffsetState() {
    ...
    if (statusEl)
      statusEl.textContent =
          '보정 시작 전 — Start 버튼을 눌러 보정을 시작하세요.';
  ...
  async startOffsetCalibration() {
    ...
    try {
      // Offset setting 시작 명령: procedure argument → procedure code 순서로
      // 전송
      await this.writeCANopenObject(
          device.slaveId, 0x2701, 0x00, 0x0001);  // procedure argument = 0x0001
      await this.writeCANopenObject(
          device.slaveId, 0x2700, 0x00, 0x0004);  // procedure code = 0x0004
  ...
  async readCurrentOffset() {
    const device = this.devices.find(d => d.id === this.currentSetupDeviceId);
    if (!device) {
      this.showToast('디바이스를 선택하세요.', 'warning');
      return;
    }
    try {
  ...
  async readHallOffset() {
    ...
    try {
      ...
      // int32_t: dataWords[0]=상위 16bit, dataWords[1]=하위 16bit → JS 비트
      // 연산으로 signed int32 자동 변환
      const fmt = r => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  deviceSetupTabs.forEach(tab => {
    ...
    tab.addEventListener('click', () => {
      ...
      if (targetTab !== 'manufacture') {
        ...
        window.dashboard
            ._tabSeq++;  // Tab Context Token 증가 — stale response 감지용
      }
      ...
      const tabContents =
          document.querySelectorAll('.device-setup-tab-content');
      ...
      if (targetTab === 'configuration') {
        ...
        document.getElementById('deviceSetupManufactureTab').style.display =
            'flex';
        // hw-overview 가 기본 서브탭 — 현재 활성 서브탭이 hw-overview면 폴링
        // 시작
        const activeSubtab =
  ...
  manufactureSubtabs.forEach(tab => {
    ...
    tab.addEventListener('click', () => {
      ...
      // 서브탭 전환: Tab Context Token 증가 — 이전 탭의 in-flight TX 응답을
      // stale로 표시
      ...
      const subtabContents =
          document.querySelectorAll('.manufacture-subtab-content');
  ...
  // Test item header click to expand/collapse (delegated — works for
  // dynamically rendered items)
  document.getElementById('osTestListContainer')
      ?.addEventListener('click', (e) => {
  ...
  // Restore Device Setup tab state from sessionStorage immediately to prevent
  // flash
  const savedPage = sessionStorage.getItem('currentPage');
  if (savedPage === 'device-setup') {
    const savedDeviceTab =
        sessionStorage.getItem('deviceSetupTab') || 'configuration';

    // Use requestAnimationFrame for smooth restoration without flash
    requestAnimationFrame(() => {
      // Find and click the saved tab (or default configuration tab) to restore
      // state
      const tabToRestore = document.querySelector(
          `.device-setup-tab[data-tab="${savedDeviceTab}"]`);
      if (tabToRestore) {
        tabToRestore.click();
      }
      ...
      if (savedDeviceTab === 'manufacture') {
        const savedManufactureSubtab =
            sessionStorage.getItem('manufactureSubtab');
        if (savedManufactureSubtab) {
          const subtabToRestore = document.querySelector(
              `.manufacture-subtab[data-subtab="${savedManufactureSubtab}"]`);
          if (subtabToRestore) {
