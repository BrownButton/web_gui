...
```
┌──────────────────┬───────────────────┬────────────┬──────────────────┐
│  Command Byte    │    Index          │ Sub-Index  │      Data        │
│  (cs) 1 byte     │   2 bytes (LE)   │   1 byte   │   4 bytes        │
└──────────────────┴───────────────────┴────────────┴──────────────────┘
```
...
```
Modbus 프레임:
01   2B   0D   40   40 60   00   00 00 00 00   [CRC_L CRC_H]
│    │    │    │    └──┬──┘  │    └────┬────┘
│    │    │    │       │     │         └── Data (읽기 요청이므로 00 패딩)
│    │    │    │       │     └──────────── Sub-Index: 0x00
...
```
Modbus 프레임:
01   2B   0D   4B   40 60   00   0F 00 00 00   [CRC_L CRC_H]
                    │    └──┬──┘  │    └────┬────┘
...
```
Modbus 프레임:
01   2B   0D   2B   40 60   00   0F 00 00 00   [CRC_L CRC_H]
                    │    └──┬──┘  │    └────┬────┘
                    │       │     │         └── Data: 0x000F (LE)
                    │       │     └──────────── Sub-Index: 0x00
                    │       └────────────────── Index: 0x6040 (LE)
...
```
Modbus 프레임:
01   2B   0D   60   40 60   00   00 00 00 00   [CRC_L CRC_H]
                    │
...
```
Modbus 프레임:
01   2B   0D   80   40 60   00   11 00 09 06   [CRC_L CRC_H]
                    │                └────┬────┘
...
**요청 및 정상 응답**: SDO PDU = 8 bytes 고정

```
전체 길이 = 1 (SlaveID) + 1 (FC) + 1 (MEI) + 8 (SDO PDU) + 2 (CRC) = 13 bytes
```
...
case 0x2B: {
    ...
    if (meiType === 0x0D) return 13;  // CANopen SDO (8 bytes PDU)
